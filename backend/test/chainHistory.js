...
exports.fixtureAddr = fixtureAddr;
exports.walkHistory = walkHistory;
...
const events_ts_1 = require("../src/events.ts");
exports.DEFAULT_SEED = 0x6c730003; // "lt3"
 ...
 */
function fixtureAddr(seed, kind, i) {
    return new web3_js_1.PublicKey((0, node_crypto_1.createHash)('sha256').update(`gc-lt3|${seed}|${kind}|${i}`).digest()).toBase58();
}
const NULL_ADDR = '11111111111111111111111111111111';
const hex32 = (x) => (x >>> 0).toString(16).padStart(8, '0').repeat(8);
const sol = (r, min, max) => String(BigInt(min + Math.floor(r() * (max - min))) * 1000000000n);
const usd = (r, min, max) => String(BigInt(min + Math.floor(r() * (max - min))) * 1000000n);
/** byte order of base58 addresses — the same comparison `fusion.rs` (and projections.ts) sorts on */
function cmpBase58(a, b) {
    const A = new web3_js_1.PublicKey(a).toBytes();
    const B = new web3_js_1.PublicKey(b).toBytes();
    for (let i = 0; i < 32; i++)
        if (A[i] !== B[i])
            return A[i] - B[i];
    return 0;
...
function* walkHistory(opts = {}) {
    ...
    const stats = { txs: 0, events: 0, byName: {}, chipsMinted: 0, chipsBurned: 0, dupes: 0, lateBlockTimes: 0, failedTxs: 0, junkTxs: 0, deferred: 0 };
    const wallets = Array.from({ length: players }, (_, i) => fixtureAddr(seed, 'w', i));
    const chips = new Map();
    const nonces = new Map();
    const stakeKeys = new Map();
    const stakedChips = new Map(); // asset → owner, for the kind-1 unstake path
    const openBattles = [];
    ...
    function* emit(events, o = {}) {
        ...
        const logs = (0, events_ts_1.fakeLogs)(events, { cpiFrom: o.cpiFrom });
        if (o.junk && noise) {
        ...
        const t = { signature: fixtureAddr(seed, 'sig', ++sigN), slot, blockTime: blockTime(), logs, err: null };
    ...
    const nonceFor = (w) => { const n = (nonces.get(w) ?? 0) + 1; nonces.set(w, n); return n; };
    const mintChip = (owner, collection, rarity, soulbound) => {
    ...
    const chipsOf = (owner, f) => [...chips.entries()].filter(([, c]) => c.owner === owner && f(c));
    const pad5 = (xs, fill) => [...xs, ...Array(Math.max(0, events_ts_1.MAX_CHIPS_PER_PACK - xs.length)).fill(fill)];
    ...
    for (iCur = 0; iCur < txsWanted && stats.events < maxEvents; iCur++) {
        ...
        if (i > 0 && i % dayEvery === 0) {
            ...
            if (rnd() < 0.3)
                yield* emit([{ program: 'staking', name: 'RootRevoked', data: { kind, epoch } }]);
            continue;
        }
        if (roll < 0.17) {
            ...
            for (let n = 0; n < qty; n++) {
                const count = 1 + Math.floor(rnd() * events_ts_1.MAX_CHIPS_PER_PACK);
            ...
            if (rnd() < 0.08)
                yield* emit([{ program: 'chip_core', name: 'PackCancelled', data: { buyer: actor, nonce: String(nonceFor(actor)), refunded: usd(rnd, 33, 200) } }]);
            continue;
        }
        if (roll < 0.22) {
        ...
        if (roll < 0.42) {
            ...
            chip.price = price;
            if (rnd() < 0.3)
                yield* emit([{ program: 'market', name: 'ListingUpdated', data: { asset, price: usd(rnd, 5, 500) } }]);
            if (rnd() < 0.2) {
                yield* emit([{ program: 'market', name: 'ListingCancelled', data: { asset } }]);
                chip.listed = false;
            }
            continue;
        }
        if (roll < 0.58) {
            ...
            if (viaOffer)
                yield* emit([{ program: 'market', name: 'OfferMade', data: { asset, bidder: buyer, amount: chip.price, expiresAt: String(blockTime() + 3600) } }], { junk: rnd() < 0.35 });
            yield* emit([
            ...
            if (viaOffer && rnd() < 0.4)
                yield* emit([{ program: 'market', name: 'OfferCancelled', data: { asset, bidder: pick(wallets.filter((w) => w !== buyer)) ?? buyer } }]);
            ...
            if (rnd() < 0.35) {
                ...
                if (other)
                    yield* emit([{ program: 'market', name: 'OfferMade', data: { asset: other[0], bidder: pick(buyers), amount: usd(rnd, 3, 400), expiresAt: String(blockTime() + 86_400) } }]);
            }
            chip.owner = buyer;
            chip.listed = false;
            continue;
        ...
        if (roll < 0.68) {
            ...
            const refund = recipe >= 4 ? new Set([...mats].sort(cmpBase58).slice(0, 1)) : new Set();
            for (const m of mats) {
                if (!success && refund.has(m))
                    continue; // refunded → survives
        ...
        if (roll < 0.8) {
            ...
            if (prev && rnd() < 0.5) {
                ...
                if (partial)
                    stakeKeys.set(key, { ...prev, amount: prev.amount - amount });
                else
                    stakeKeys.delete(key);
                continue;
            }
            const add = BigInt(usd(rnd, 1, 400)) + (prev?.amount ?? 0n);
            stakeKeys.set(key, { owner: actor, amount: add });
            yield* emit([{ program: 'staking', name: 'Staked', data: { owner: actor, kind: 0, key, amount: add.toString(), weight: String(add / 1000000n), unlockAt: String(blockTime() + 86_400 * (1 + Math.floor(rnd() * 30))) } }]);
            continue;
        }
        if (roll < 0.86) {
            ...
            if (openBattles.length > 0 && rnd() < 0.6) {
                ...
                if (rnd() < 0.12) {
                    yield* emit([{ program: 'arena', name: 'BattleCancelled', data: { battle, refundedA: usd(rnd, 1, 50), refundedB: '0' } }]);
                    drop();
                    continue;
                }
                yield* emit([{ program: 'arena', name: 'BattleAccepted', data: { battle, opponent: pick(wallets), powerB: 100 + Math.floor(rnd() * 900) } }]);
                const rake = BigInt(Math.floor(rnd() * 19 + 1)) * 1000000n;
            ...
            if (rnd() < 0.06)
                yield* emit([{ program: 'arena', name: 'BattleResolved', data: { battle: fixtureAddr(seed, 'battle', 1_000_000 + i), winner: actor, pot: usd(rnd, 1, 2), rakeBurn: usd(rnd, 0, 1), rakePool: usd(rnd, 1, 2), rakeTreasury: usd(rnd, 1, 2), resultHash: hex32(i), roll: hex32(i) } }]);
            continue;
        }
        if (roll < 0.92) {
            const kind = Math.floor(rnd() * 10);
            ...
            if (rnd() < 0.25)
                yield* emit([{ program: 'chip_core', name: 'ServicePaid', data: { buyer: actor, kind: 0, currency: 2, amount: '199000000', burned: '199000000', refHash: hex32(i) } }]);
            continue;
        }
        // admin + treasury bookkeeping
        yield* emit([{ program: pick(['chip_core', 'staking', 'arena']), name: 'PauseChanged', data: { by: actor, paused: rnd() < 0.5 } }]);
        if (rnd() < 0.5) {
        ...
        if (rnd() < 0.3)
            yield* emit([{ program: 'staking', name: 'SkrPoolChanged', data: { maxRootBudget: usd(rnd, 100, 5000), paused: rnd() < 0.1 } }]);
        if (lastRoot)
            yield* emit([{ program: 'staking', name: 'SliceFunded', data: { by: actor, kind: lastRoot.kind, amount: usd(rnd, 1, 60), sliceBudget: Array.from({ length: 5 }, () => usd(rnd, 0, 40)), recycledTotal: usd(rnd, 0, 100) } }]);
        if (rnd() < 0.4)
            yield* emit([{ program: 'staking', name: 'SetBonusSynced', data: { owner: actor, sets: Math.floor(rnd() * 8) } }]);
        // `source` is a pubkey per the event spec (which burn account fed the vault), so it takes an address
        if (rnd() < 0.4)
            yield* emit([{ program: 'staking', name: 'BurnRecorded', data: { source: fixtureAddr(seed, 'burnsrc', Math.floor(rnd() * 4)), amount: usd(rnd, 1, 20), burnToday: usd(rnd, 1, 400) } }]);
        if (rnd() < 0.3) {
            paramVersion++;
            yield* emit([{ program: 'chip_core', name: 'ParamsChanged', data: { admin: wallets[0], version: paramVersion } }]);
        }
        ...
        if (rnd() < 0.25)
            yield* emit([{ program: pick(['chip_core', 'staking', 'arena']), name: 'PauserChanged', data: { by: wallets[0], pauser: rnd() < 0.2 ? NULL_ADDR : fixtureAddr(seed, 'pauser', Math.floor(rnd() * 3)) } }]);
        if (rnd() < 0.15) {
            ...
            if (rnd() < 0.5)
                yield* emit([{ program: 'chip_core', name: 'AdminAccepted', data: { oldAdmin: wallets[0], newAdmin } }]);
        }
        if (rnd() < 0.15)
            yield* emit([{ program: 'staking', name: 'OraclesChanged', data: { by: wallets[0], questOracle: fixtureAddr(seed, 'oracle', 0), seasonOracle: fixtureAddr(seed, 'oracle', 1), setOracle: fixtureAddr(seed, 'oracle', 2), burnOracle: rnd() < 0.3 ? NULL_ADDR : fixtureAddr(seed, 'oracle', 3) } }]);
        if (rnd() < 0.15)
            yield* emit([{ program: 'arena', name: 'ArenaConfigChanged', data: { by: wallets[0], battleOracle: fixtureAddr(seed, 'oracle', 4), oracleDailyCap: usd(rnd, 100_000, 200_000), treasuryCg: fixtureAddr(seed, 'treasury', 0) } }]);
        if (rnd() < 0.1)
            yield* emit([{ program: 'chip_core', name: 'CollectionCreated', data: { by: wallets[0], idx: Math.floor(rnd() * 8), coreCollection: fixtureAddr(seed, 'collection', Math.floor(rnd() * 8)) } }]);
        if (!noise)
            continue;
        ...
        if (i % DEFER_EVERY === 41 && deferred.length > 0)
            yield deferred.shift();
