...
const ctx = new node_async_hooks_1.AsyncLocalStorage();
/** The id / wallet the current request is associated with (empty outside a request). */
function logContext() { return ctx.getStore() ?? {}; }
function runWithContext(fields, fn) { return ctx.run({ ...logContext(), ...fields }, fn); }
/**
 ...
 */
function safeValue(v, depth = 0, seen = new WeakSet()) {
    ...
    if (t === 'number' || t === 'boolean')
        return v;
    if (t === 'string')
        return v.length > 2_000 ? `${v.slice(0, 2_000)}…+${v.length - 2_000} chars` : v;
    if (t === 'function')
        return `[fn ${v.name ?? 'anonymous'}]`;
    if (t === 'symbol' || t === 'undefined')
        return String(v);
    if (t !== 'object')
        return String(v);
    const o = v;
    if (seen.has(o))
        return '[circular]';
    if (depth > 3)
        return Array.isArray(v) ? `[array ${v.length}]` : '[depth]';
    seen.add(o);
    if (v instanceof Error)
        return { message: v.message, ...v.code ? { code: v.code } : {} };
    if (Array.isArray(v))
        return v.slice(0, 24).map((x) => safeValue(x, depth + 1, seen));
    if (v instanceof Map)
        return Object.fromEntries([...v.entries()].slice(0, 24).map(([k, x]) => [String(k), safeValue(x, depth + 1, seen)]));
    if (v instanceof Set)
        return [...v].slice(0, 24).map((x) => safeValue(x, depth + 1, seen));
    const out = {};
    let n = 0;
    for (const [k, x] of Object.entries(v)) {
        ...
        try {
            out[k] = safeValue(x, depth + 1, seen);
        }
        catch {
            out[k] = '[throw]';
        }
...
function safeJson(fields) {
    try {
        return JSON.stringify(safeValue(fields));
    }
    catch {
...
function emit(level, msg, fields, stream = process.stdout) {
...
exports.log = {
    ...
    warn: (msg, fields) => emit('warn', msg, fields, process.stderr),
    error: (msg, fields) => emit('error', msg, fields, process.stderr),
    /** One ALERT line that both the log pipeline and a human on the box can grep. Keep the exact word. */
    alert: (msg, fields) => emit('error', `ALERT ${msg}`, { alert: true, ...fields }, process.stderr),
};
...
function newRequestId() { return (0, node_crypto_1.randomUUID)().replace(/-/g, '').slice(0, 16); }
/**
...
function hijackConsole() {
    console.log = (...a) => exports.log.info(a.map((x) => (typeof x === 'string' ? x : JSON.stringify(x))).join(' '));
    console.info = console.log;
    console.warn = (...a) => exports.log.warn(a.map((x) => (typeof x === 'string' ? x : JSON.stringify(x))).join(' '));
    console.error = (...a) => exports.log.error(a.map((x) => (typeof x === 'string' ? x : JSON.stringify(x))).join(' '));
}
