...
exports.HANDLED_EVENTS = exports.WALLET_TOUCH_FIELDS = void 0;
exports.patchLateTimes = patchLateTimes;
 ...
 */
function touchWallet(db, address, c) {
    db.run(`INSERT INTO wallets (address, first_seen) VALUES (?, ?)
     ON CONFLICT(address) DO UPDATE SET first_seen = COALESCE(
       ...
       wallets.first_seen, excluded.first_seen)`, address, c.blockTime);
}
function setChipFlag(db, asset, flag, on, slot) {
    db.run(on ? `UPDATE chips SET flags = flags | ?, updated_slot = ? WHERE asset = ?` : `UPDATE chips SET flags = flags & ~?, updated_slot = ? WHERE asset = ?`, flag, slot, asset);
}
const HANDLERS = {
    // ------------------------------------------------------------ chip_core
    ServicePaid(db, e, c) {
        const d = e.data;
        touchBySpec(db, e, c);
        db.run((0, sql_ts_1.insertIgnore)('service_payments', COLS.servicePayments), c.signature, e.eventIndex, str(d.buyer), num(d.kind), num(d.currency), str(d.amount), str(d.burned), str(d.refHash), c.slot, c.blockTime);
    },
    PackBought(db, e, c) {
        const d = e.data;
        touchBySpec(db, e, c);
        db.run((0, sql_ts_1.insertIgnore)('pack_purchases', COLS.packPurchases), str(d.buyer), str(d.nonce), num(d.sku), num(d.qty), num(d.currency), str(d.amount), str(d.randomness), c.signature, c.slot, c.blockTime);
    },
    /** (#28) A free quest-chip PendingPack — tracked apart from purchases; its `PackOpened` arrives with `sku = 0`. */
    VoucherIssued(db, e, c) {
        const d = e.data;
        touchBySpec(db, e, c);
        db.run((0, sql_ts_1.insertIgnore)('vouchers', COLS.vouchers), str(d.wallet), str(d.nonce), num(d.template), str(d.randomness), c.signature, c.slot, c.blockTime);
    },
    CompressedClaimsCreated(db, e, c) {
        const d = e.data;
        const buyer = str(d.buyer);
        const nonce = str(d.nonce);
        const claimNonces = d.claimNonces;
        const count = num(d.count);
        touchBySpec(db, e, c);
        db.run(`INSERT INTO compressed_settlements (buyer, nonce, last_signature, last_slot, block_time)
       VALUES (?, ?, ?, ?, ?)
       ON CONFLICT(buyer, nonce) DO UPDATE SET last_signature = excluded.last_signature,
         last_slot = excluded.last_slot, block_time = COALESCE(excluded.block_time, compressed_settlements.block_time)`, buyer, nonce, c.signature, c.slot, c.blockTime);
        for (let i = 0; i < count; i++) {
            db.run(`INSERT INTO compressed_claims (buyer, nonce, claim_nonce, pack_no, slot, block_time)
         VALUES (?, ?, ?, ?, ?, ?)
         ON CONFLICT(buyer, nonce, claim_nonce) DO UPDATE SET pack_no = excluded.pack_no,
           slot = excluded.slot, block_time = COALESCE(excluded.block_time, compressed_claims.block_time)`, buyer, nonce, claimNonces[i], num(d.packNo), c.slot, c.blockTime);
        }
        db.run(`UPDATE compressed_settlements
       SET total_claims = (SELECT COUNT(*) FROM compressed_claims WHERE buyer = ? AND nonce = ?)
       WHERE buyer = ? AND nonce = ?`, buyer, nonce, buyer, nonce);
    },
    CompressedClaimCancelled(db, e, c) {
        ...
        touchBySpec(db, e, c);
        const wasPending = db.scalar(`SELECT COUNT(*) FROM compressed_claims WHERE buyer = ? AND nonce = ? AND claim_nonce = ? AND status <> 'cancelled'`, buyer, nonce, claimNonce);
        db.run(`UPDATE compressed_claims SET status = 'cancelled', slot = ?, block_time = COALESCE(?, block_time)
       WHERE buyer = ? AND nonce = ? AND claim_nonce = ?`, c.slot, c.blockTime, buyer, nonce, claimNonce);
        if (wasPending) {
    ...
    CompressedChipMinted(db, e, c) {
        ...
        const claimNonce = str(d.claimNonce);
        touchBySpec(db, e, c);
        db.run(`UPDATE compressed_claims SET status = CASE WHEN status IN ('registered', 'cancelled') THEN status ELSE 'minted' END,
         collection_idx = ?, rarity = ?, level = ?, game_index = ?, mint_signature = ?, slot = ?,
         block_time = COALESCE(?, block_time)
       WHERE buyer = ? AND claim_nonce = ?`, num(d.collectionIdx), num(d.rarity), num(d.level), str(d.gameIndex), c.signature, c.slot, c.blockTime, buyer, claimNonce);
    },
    CompressedChipRegistered(db, e, c) {
        ...
        const claimNonce = str(d.claimNonce);
        touchBySpec(db, e, c);
        const previous = db.get(`SELECT status FROM compressed_claims WHERE buyer = ? AND claim_nonce = ?`, buyer, claimNonce);
        const wasRegistered = previous?.status === 'registered';
        db.run(`UPDATE compressed_claims SET status = CASE WHEN status = 'cancelled' THEN status ELSE 'registered' END, asset = ?, collection_idx = ?, rarity = ?, level = ?,
         register_signature = ?, slot = ?, block_time = COALESCE(?, block_time)
       WHERE buyer = ? AND claim_nonce = ?`, str(d.asset), num(d.collectionIdx), num(d.rarity), num(d.level), c.signature, c.slot, c.blockTime, buyer, claimNonce);
        ...
        db.run((0, sql_ts_1.upsert)('chips', COLS.chips, ['asset'], ['owner = excluded.owner', 'collection_idx = excluded.collection_idx', 'rarity = excluded.rarity', 'level = excluded.level', 'flags = excluded.flags', 'lock_until = excluded.lock_until', 'updated_slot = excluded.updated_slot', 'origin_signature = excluded.origin_signature', 'minted_at = COALESCE(chips.minted_at, excluded.minted_at)']), str(d.asset), buyer, num(d.collectionIdx), num(d.rarity), num(d.level), num(d.flags), Number(d.lockUntil), 'compressed', c.signature, c.blockTime, c.slot);
    },
    CompressedPackSettled(db, e, c) {
        ...
        const nonce = str(d.nonce);
        touchBySpec(db, e, c);
        db.run(`UPDATE compressed_settlements SET status = ?, last_signature = ?, last_slot = ?, block_time = COALESCE(?, block_time)
       WHERE buyer = ? AND nonce = ?`, d.refunded ? 'refunded' : 'settled', c.signature, c.slot, c.blockTime, buyer, nonce);
    },
    PackOpened(db, e, c) {
        const d = e.data;
        const count = num(d.count);
        const assets = d.assets.slice(0, count);
        const rarities = d.rarities.slice(0, count);
        const collections = d.collections.slice(0, count);
        const buyer = str(d.buyer);
        touchBySpec(db, e, c);
        db.run((0, sql_ts_1.insertIgnore)('pack_opens', COLS.packOpens), c.signature, buyer, num(d.sku), str(d.nonce), count, j(assets), j(rarities), j(collections), str(d.roll), num(d.pityBefore), num(d.pityAfter), c.slot, c.blockTime);
        // (#28) a voucher open carries sku 0 too — the (wallet, nonce) tells them apart; its lock comes from the template
        const voucher = num(d.sku) === 0 ? db.get(`SELECT template FROM vouchers WHERE wallet = ? AND nonce = ?`, buyer, str(d.nonce)) : undefined;
        const soulboundDays = voucher ? (economy_1.QUEST_CHIP_TEMPLATES[voucher.template]?.soulboundDays ?? 0) : num(d.sku) === 0 ? 7 : 0; // Starter: 7 days (chip_core packs.rs)
        ...
        for (let i = 0; i < count; i++) {
            ...
            (0, sql_ts_1.upsert)('chips', COLS.chips, ['asset'], [
        ...
        if (voucher)
            db.run(`UPDATE vouchers SET status = 'opened' WHERE wallet = ? AND nonce = ?`, buyer, str(d.nonce));
        else
    ...
    ChipFused(db, e, c) {
        ...
        touchBySpec(db, e, c);
        db.run((0, sql_ts_1.insertIgnore)('fusions', COLS.fusions), c.signature, e.eventIndex, owner, num(d.recipe), j(materials), hasResult ? result : null, success ? 1 : 0, num(d.rollBps), num(d.thresholdBps), str(d.feeBurned), c.slot, c.blockTime);
        // The commit half sets F_FUSING (ChipFlagsChanged is not emitted for it), the settle half clears it here.
        if (success) {
            for (const m of materials)
                db.run(`UPDATE chips SET burned_at = ?, flags = flags & ~?, updated_slot = ? WHERE asset = ?`, c.blockTime ?? 0, CHIP_FLAG_FUSING, c.slot, m);
        }
        else {
            ...
            for (const m of materials) {
                if (survivors.has(m))
                    db.run(`UPDATE chips SET flags = flags & ~?, updated_slot = ? WHERE asset = ?`, CHIP_FLAG_FUSING, c.slot, m);
                else
                    db.run(`UPDATE chips SET burned_at = ?, flags = flags & ~?, updated_slot = ? WHERE asset = ?`, c.blockTime ?? 0, CHIP_FLAG_FUSING, c.slot, m);
            }
        }
        if (hasResult) {
            const mat = db.get(`SELECT collection_idx, rarity FROM chips WHERE asset = ?`, materials[0]);
            const rarity = mat ? mat.rarity + 1 : num(d.recipe) + 1;
            const collection = mat?.collection_idx ?? 0;
            db.run((0, sql_ts_1.upsert)('chips', COLS.chips, ['asset'], ['owner = excluded.owner', 'updated_slot = excluded.updated_slot']), result, owner, collection, rarity, 1, 0, 0, 'fusion', c.signature, c.blockTime, c.slot);
        }
    },
    /**
    ...
    CompressedClaimsFused(db, e, c) {
        const d = e.data;
        touchBySpec(db, e, c);
        db.run((0, sql_ts_1.insertIgnore)('fusions', COLS.fusions), c.signature, e.eventIndex, str(d.owner), num(d.recipe), j(d.materials), str(d.resultClaim), 1, 0, 10_000, str(d.feeBurned), c.slot, c.blockTime);
    },
    ...
    ClaimFusionRevealed(db, e, c) {
        ...
        touchBySpec(db, e, c);
        db.run((0, sql_ts_1.insertIgnore)('fusions', COLS.fusions), c.signature, e.eventIndex, str(d.owner), num(d.recipe), j(d.materials), success && result !== '11111111111111111111111111111111' ? result : null, success ? 1 : 0, num(d.rollBps), num(d.thresholdBps), str(d.feeBurned), c.slot, c.blockTime);
    },
    ...
    ParamsChanged(db, e, c) {
        const d = e.data;
        db.run((0, sql_ts_1.insertIgnore)('params_changes', COLS.paramsChanges), c.signature, str(d.admin), num(d.version), c.slot, c.blockTime);
    },
    /** SEC-H2 audit trail: who paused/un-paused which program and when (`e.program` = chip_core | staking | arena). */
    PauseChanged(db, e, c) {
        const d = e.data;
        db.run((0, sql_ts_1.insertIgnore)('pause_changes', COLS.pauseChanges), c.signature, e.eventIndex, e.program, str(d.by), d.paused ? 1 : 0, c.slot, c.blockTime);
    },
    ...
    BurnReported(db, e, c) {
        const d = e.data;
        db.run((0, sql_ts_1.insertIgnore)('burns', COLS.burns), c.signature, e.eventIndex, 'chip_core', str(d.source), str(d.amount), c.slot, c.blockTime);
    },
    // ------------------------------------------------------------ market
    ChipListed(db, e, c) {
        const d = e.data;
        touchBySpec(db, e, c);
        db.run((0, sql_ts_1.upsert)('listings', ['asset', 'seller', 'price', 'currency', 'created_at', 'slot', 'signature'], ['asset'], [
        ...
        db.run((0, sql_ts_1.insertIgnore)('burns', COLS.burns), c.signature, e.eventIndex, 'market', 'listing_fee', String(economy_1.FEES.listingFeeCgMicro), c.slot, c.blockTime);
    },
    ...
    ChipSold(db, e, c) {
        ...
        const chip = db.get(`SELECT collection_idx, rarity FROM chips WHERE asset = ?`, asset);
        touchBySpec(db, e, c);
        db.run((0, sql_ts_1.insertIgnore)('sales', COLS.sales), c.signature, e.eventIndex, asset, str(d.seller), str(d.buyer), str(d.price), num(d.currency), str(d.fee), str(d.royalty), d.viaOffer ? 1 : 0, chip?.collection_idx ?? null, chip?.rarity ?? null, c.slot, c.blockTime);
    ...
    OfferMade(db, e, c) {
        const d = e.data;
        touchBySpec(db, e, c);
        db.run((0, sql_ts_1.upsert)('offers', ['asset', 'bidder', 'amount', 'expires_at', 'slot'], ['asset', 'bidder'], [
    ...
    BattleCreated(db, e, c) {
        const d = e.data;
        touchBySpec(db, e, c);
        db.run((0, sql_ts_1.insertIgnore)('battles', COLS.battlesCreated), str(d.battle), str(d.challenger), str(d.wager), num(d.powerA), str(d.randomness), c.signature, c.blockTime, c.slot);
    },
    ...
    BattleResolved(db, e, c) {
        const d = e.data;
        db.run(`UPDATE battles SET winner = ?, pot = ?, rake_burn = ?, rake_pool = ?, rake_treasury = ?, result_hash = ?, roll = ?, status = 'resolved', resolved_sig = ?, resolved_at = ?, slot = ? WHERE battle = ?`, str(d.winner), str(d.pot), str(d.rakeBurn), str(d.rakePool), str(d.rakeTreasury), str(d.resultHash), str(d.roll), c.signature, c.blockTime, c.slot, str(d.battle));
        // A resolved battle we never saw created (backfill gap) still counts for the leaderboard.
        db.run((0, sql_ts_1.insertIgnore)('battles', COLS.battlesResolved), str(d.battle), str(d.winner), '0', 0, '', str(d.winner), str(d.pot), str(d.rakeBurn), str(d.rakePool), str(d.rakeTreasury), str(d.resultHash), str(d.roll), 'resolved', c.signature, c.signature, c.blockTime, c.slot);
        // the burned rake slice (40 % of 5 %) feeds the emission guard through the burn oracle (SEC-M1)
        if (BigInt(str(d.rakeBurn)) > 0n) {
            db.run((0, sql_ts_1.insertIgnore)('burns', COLS.burns), c.signature, e.eventIndex, 'arena', 'rake_burn', str(d.rakeBurn), c.slot, c.blockTime);
        }
    },
    ...
    DayClosed(db, e, c) {
        const d = e.data;
        db.run((0, sql_ts_1.insertIgnore)('emission_days', COLS.emissionDays), num(d.dayIndex), num(d.year), str(d.scheduleCap), str(d.guarded), str(d.burn7dAvg), j(d.sliceBudget), c.signature, c.blockTime);
    },
    Staked(db, e, c) {
        const d = e.data;
        const owner = str(d.owner);
        touchBySpec(db, e, c);
        db.run((0, sql_ts_1.upsert)('stakes', ['key', 'owner', 'kind', 'amount', 'weight', 'unlock_at', 'since', 'slot', 'active'], ['key'], [
        ...
        if (num(d.kind) === 1)
            setChipFlag(db, str(d.key), CHIP_FLAG_STAKED, true, c.slot);
    },
    Unstaked(db, e, c) {
        const d = e.data;
        if (num(d.kind) === 1) {
            ...
            const row = db.get(`SELECT amount FROM stakes WHERE key = ?`, str(d.key));
            const left = row ? BigInt(row.amount) - BigInt(str(d.amount)) : 0n;
            if (left <= 0n)
                db.run(`UPDATE stakes SET amount = '0', active = 0, slot = ? WHERE key = ?`, c.slot, str(d.key));
            else
                db.run(`UPDATE stakes SET amount = ?, slot = ? WHERE key = ?`, left.toString(), c.slot, str(d.key));
        }
        if (BigInt(str(d.penaltyBurned)) > 0n) {
            db.run((0, sql_ts_1.insertIgnore)('burns', COLS.burns), c.signature, e.eventIndex, 'staking', 'early_exit', str(d.penaltyBurned), c.slot, c.blockTime);
        }
    },
    Claimed(db, e, c) {
        const d = e.data;
        db.run((0, sql_ts_1.insertIgnore)('claims', COLS.claims), c.signature, e.eventIndex, str(d.owner), num(d.kind), str(d.amount), c.slot, c.blockTime);
    },
    RootPublished(db, e, c) {
        const d = e.data;
        db.run((0, sql_ts_1.upsert)('reward_roots', ['kind', 'epoch', 'currency', 'root', 'budget', 'revoked', 'signature', 'slot'], ['kind', 'epoch'], [
            'root = excluded.root', 'budget = excluded.budget', 'revoked = 0', 'signature = excluded.signature', 'slot = excluded.slot',
        ]), num(d.kind), num(d.epoch), (0, events_ts_1.rootCurrency)(num(d.kind)), str(d.root), str(d.budget), 0, c.signature, c.slot);
    },
    RootRevoked(db, e, c) {
        const d = e.data;
    ...
    RootClaimed(db, e, c) {
        const d = e.data;
        touchBySpec(db, e, c);
        db.run((0, sql_ts_1.insertIgnore)('reward_claims', COLS.rewardClaims), num(d.kind), num(d.epoch), (0, events_ts_1.rootCurrency)(num(d.kind)), str(d.wallet), str(d.amount), c.signature, c.slot);
    },
    SliceFunded(db, e, c) {
        const d = e.data;
        db.run((0, sql_ts_1.insertIgnore)('slice_fundings', COLS.sliceFundings), c.signature, e.eventIndex, str(d.by), num(d.kind), str(d.amount), j(d.sliceBudget), str(d.recycledTotal), c.slot, c.blockTime);
        // not a `burns` row on purpose: the tokens come back at claim (recycled), so the guard ring must not see demand here
    },
    SkrFunded(db, e, c) {
        const d = e.data;
        db.run((0, sql_ts_1.insertIgnore)('skr_pool_events', COLS.skrFunded), c.signature, e.eventIndex, 'funded', str(d.funder), str(d.amount), str(d.budget), str(d.reserved), c.slot, c.blockTime);
    },
    SkrWithdrawn(db, e, c) {
        const d = e.data;
        db.run((0, sql_ts_1.insertIgnore)('skr_pool_events', COLS.skrWithdrawn), c.signature, e.eventIndex, 'withdrawn', str(d.to), str(d.amount), str(d.budget), c.slot, c.blockTime);
    },
    SkrPoolChanged(db, e, c) {
        const d = e.data;
        db.run((0, sql_ts_1.insertIgnore)('skr_pool_events', COLS.skrChanged), c.signature, e.eventIndex, 'changed', str(d.maxRootBudget), d.paused ? 1 : 0, c.slot, c.blockTime);
    },
    BurnRecorded(db, e, c) {
        const d = e.data;
        db.run((0, sql_ts_1.insertIgnore)('burns', COLS.burns), c.signature, e.eventIndex, 'staking', str(d.source), str(d.amount), c.slot, c.blockTime);
    },
    SetBonusSynced(db, e, c) {
        const d = e.data;
        db.run((0, sql_ts_1.upsert)('set_bonus', ['owner', 'sets', 'slot'], ['owner'], ['sets = excluded.sets', 'slot = excluded.slot']), str(d.owner), num(d.sets), c.slot);
    },
};
/**
...
function touchBySpec(db, e, c) {
    for (const field of exports.WALLET_TOUCH_FIELDS[e.name] ?? []) {
 ...
 */
function patchLateTimes(db, e, c) {
    if (c.blockTime === null)
        return 0;
    const sig = c.signature, blockTime = c.blockTime, d = e.data;
    let n = 0;
    const changed = (sql, ...params) => Number(db.run(sql, ...params).changes);
    const fill = (table, col, keyCol = 'signature') => {
        n += changed(`UPDATE ${table} SET ${col} = ? WHERE ${keyCol} = ? AND ${col} IS NULL`, blockTime, sig);
    };
    switch (e.name) {
        case 'ServicePaid':
            fill('service_payments', 'block_time');
            break;
        case 'PackBought':
            fill('pack_purchases', 'block_time');
            break;
        case 'VoucherIssued':
            fill('vouchers', 'block_time');
            break;
        case 'PackOpened':
            ...
            if (num(d.sku) === 0) {
                const v = db.get(`SELECT template FROM vouchers WHERE wallet = ? AND nonce = ?`, str(d.buyer), str(d.nonce));
                const days = v ? (economy_1.QUEST_CHIP_TEMPLATES[v.template]?.soulboundDays ?? 0) : 7;
                if (days > 0)
                    n += changed(`UPDATE chips SET lock_until = minted_at + ? WHERE origin_signature = ? AND lock_until = 0 AND (flags & 8) <> 0 AND minted_at IS NOT NULL`, days * 86_400, sig);
            }
            break;
        case 'ChipFused': {
            ...
            const mats = (e.data.materials ?? []).filter((m) => typeof m === 'string' && m.length > 0);
            if (mats.length > 0) {
        ...
        case 'ChipListed':
            ...
            break;
        case 'ChipSold':
            fill('sales', 'block_time');
            break;
        case 'BattleCreated':
            fill('battles', 'created_at', 'created_sig');
            break;
        case 'BattleResolved':
        ...
        case 'Unstaked':
            ...
            break;
        case 'Claimed':
            fill('claims', 'block_time');
            break;
        case 'DayClosed':
            fill('emission_days', 'block_time');
            break;
        case 'SliceFunded':
            fill('slice_fundings', 'block_time');
            break;
        case 'SkrFunded':
        case 'SkrWithdrawn':
        case 'SkrPoolChanged':
            fill('skr_pool_events', 'block_time');
            break;
        case 'BurnReported':
        case 'BurnRecorded':
            fill('burns', 'block_time');
            break;
        case 'ParamsChanged':
            fill('params_changes', 'block_time');
            break;
        case 'PauseChanged':
            fill('pause_changes', 'block_time');
            break;
...
function authorityChange(db, e, c, roles, byField) {
    const d = e.data;
    const detail = JSON.stringify(d);
    for (const [kind, field] of roles) {
        db.run((0, sql_ts_1.insertIgnore)('authority_changes', COLS.authorityChanges), c.signature, e.eventIndex, e.program, kind, str(d[byField]), str(d[field]), detail, c.slot, c.blockTime);
    }
}
/** Compare two base58 pubkeys by their byte representation (what fusion.rs sorts on). */
function cmpBase58Bytes(a, b) {
    const A = new web3_js_1.PublicKey(a).toBytes(), B = new web3_js_1.PublicKey(b).toBytes();
    for (let i = 0; i < 32; i++)
        if (A[i] !== B[i])
            return A[i] - B[i];
    return 0;
}
function applyEvent(db, e, c) {
...
exports.HANDLED_EVENTS = Object.keys(HANDLERS);
