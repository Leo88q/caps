...
export class BorshReader {
  ...
  private need(n: number) { if (this.offset + n > this.buf.length) throw new RangeError(`borsh: need ${n} bytes at ${this.offset}, have ${this.remaining}`); }
  ...
  pubkey() { this.need(32); const k = new PublicKey(this.buf.subarray(this.offset, this.offset + 32)); this.offset += 32; return k; }
  bytes(n: number) { this.need(n); const b = this.buf.slice(this.offset, this.offset + n); this.offset += n; return b; }
