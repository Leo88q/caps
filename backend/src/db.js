...
exports.now = exports.Db = exports.PROJECTION_TABLES = exports.SCHEMA = void 0;
exports.db = db;
exports.useDb = useDb;
...
class Db {
    raw;
    stmts = new Map();
    constructor(path = config_ts_1.DB_PATH) {
        this.raw = new DatabaseSync(path);
        this.raw.exec(exports.SCHEMA);
        this.migrate();
    }
    /** Additive, idempotent column migrations for dev SQLite files created by older builds. */
    migrate() {
        const nonces = new Set(this.raw.prepare(`PRAGMA table_info(siws_nonces)`).all().map((c) => c.name));
        if (!nonces.has('consumed_at'))
            this.raw.exec(`ALTER TABLE siws_nonces ADD COLUMN consumed_at INTEGER`);
        const cols = new Set(this.raw.prepare(`PRAGMA table_info(oracle_prices)`).all().map((c) => c.name));
        for (const [name, type] of [['publish_time', 'INTEGER'], ['account', 'TEXT'], ['conf_bps', 'INTEGER']]) {
            if (!cols.has(name))
                this.raw.exec(`ALTER TABLE oracle_prices ADD COLUMN ${name} ${type}`);
        }
        const ev = new Set(this.raw.prepare(`PRAGMA table_info(events_raw)`).all().map((c) => c.name));
        if (!ev.has('finalized_at'))
            this.raw.exec(`ALTER TABLE events_raw ADD COLUMN finalized_at INTEGER`);
        this.raw.exec(`CREATE INDEX IF NOT EXISTS idx_events_unfinalized ON events_raw(finalized_at, slot)`);
        const ch = new Set(this.raw.prepare(`PRAGMA table_info(chips)`).all().map((c) => c.name));
        if (!ch.has('skin'))
            this.raw.exec(`ALTER TABLE chips ADD COLUMN skin TEXT`);
        const ql = new Set(this.raw.prepare(`PRAGMA table_info(quest_logins)`).all().map((c) => c.name));
        if (!ql.has('minute_of_day'))
            this.raw.exec(`ALTER TABLE quest_logins ADD COLUMN minute_of_day INTEGER`);
        const se = new Set(this.raw.prepare(`PRAGMA table_info(seasons)`).all().map((c) => c.name));
        for (const [name, type] of [['rake_micro', 'TEXT'], ['rake_funded_at', 'INTEGER'], ['rake_funded_sig', 'TEXT']]) {
            if (!se.has(name))
                this.raw.exec(`ALTER TABLE seasons ADD COLUMN ${name} ${type}`);
        }
        const qc = new Set(this.raw.prepare(`PRAGMA table_info(quest_completions)`).all().map((c) => c.name));
        if (!qc.has('day')) {
        ...
        for (const name of ['item_root_kind', 'item_root_epoch']) {
            if (!qc.has(name))
                this.raw.exec(`ALTER TABLE quest_completions ADD COLUMN ${name} INTEGER`);
        }
        ...
        for (const name of ['chip_root_kind', 'chip_root_epoch']) {
            if (!qc.has(name))
                this.raw.exec(`ALTER TABLE quest_completions ADD COLUMN ${name} INTEGER`);
        }
    ...
    run(sql, ...params) { return this.prep(sql).run(...params); }
    get(sql, ...params) { return this.prep(sql).get(...params); }
    all(sql, ...params) { return this.prep(sql).all(...params); }
...
function useDb(instance) { shared = instance; }
