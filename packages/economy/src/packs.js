...
exports.STANDARD_EV_TARGET = exports.rarityName = exports.BUNDLES = exports.STALE_PACK_MINUTES = exports.STALE_PACK_SLOTS = exports.PACKS = void 0;
exports.bundlePriceCents = bundlePriceCents;
exports.effectiveOdds = effectiveOdds;
exports.rollRarity = rollRarity;
exports.uniformBps = uniformBps;
exports.uniformPool = uniformPool;
exports.expandRandomness = expandRandomness;
exports.packExpectedValueMult = packExpectedValueMult;
exports.probabilityAtLeast = probabilityAtLeast;
exports.impliedCommonFloorUsd = impliedCommonFloorUsd;
 ...
 */
const STANDARD_ODDS = [4500, 2500, 1500, 800, 450, 180, 50, 18, 2];
...
const PREMIUM_ODDS = [2800, 2600, 2250, 1400, 600, 250, 70, 25, 5];
...
const STARTER_ODDS = [3000, 3000, 2500, 1100, 350, 50, 0, 0, 0];
...
const LIMITED_ODDS = [2200, 2400, 2400, 1600, 850, 350, 120, 50, 30];
exports.PACKS = {
    starter: {
 ...
 */
exports.STALE_PACK_SLOTS = 10_800;
exports.STALE_PACK_MINUTES = Math.round((exports.STALE_PACK_SLOTS * 0.4) / 60); // ≈ 72
...
function bundlePriceCents(pack, qty) {
    const tier = [...exports.BUNDLES].reverse().find((b) => qty >= b.qty) ?? exports.BUNDLES[0];
...
function effectiveOdds(pack, pityCounter) {
...
function rollRarity(roll, oddsBps) {
    let acc = 0;
...
function uniformBps(vrf, slot) {
    ...
    let fold = 0n;
    for (const b of vrf)
        fold = (fold * 31n + BigInt(b)) & 0xffffffffffffffffn;
 ...
 */
function uniformPool(vrf, slot, pool) {
    if (pool <= 1)
        return 0;
 ...
 */
function expandRandomness(vrf, pack, pityCounter, poolSize) {
    if (vrf.length < 32)
        throw new Error('VRF output must be 32 bytes');
    ...
    for (let i = 0; i < pack.chips; i++) {
        ...
        if (isLast && rarity < pack.floor)
            rarity = pack.floor;
        if (isLast && pack.pity && pityCounter + 1 >= pack.pity.hardAt && rarity < pack.pity.tier)
            rarity = pack.pity.tier;
...
// -----------------------------------------------------------------------------
function packExpectedValueMult(pack) {
    const odds = pack.oddsBps;
    const evSlot = odds.reduce((acc, bps, i) => acc + (bps / 10_000) * rarity_ts_1.RARITY_PROFILES[i].valueMult, 0);
    // floor on the last slot: mass below floor collapses into floor
    const below = odds.slice(0, pack.floor).reduce((a, b) => a + b, 0) / 10_000;
    const evBelow = odds.slice(0, pack.floor).reduce((acc, bps, i) => acc + (bps / 10_000) * rarity_ts_1.RARITY_PROFILES[i].valueMult, 0);
    const evLast = evSlot - evBelow + below * rarity_ts_1.RARITY_PROFILES[pack.floor].valueMult;
    return evSlot * (pack.chips - 1) + evLast;
}
function probabilityAtLeast(pack, tier) {
...
const rarityName = (i) => rarity_ts_1.RARITIES[i];
exports.rarityName = rarityName;
/** Target EV/price for the anchor SKU; defines the modelled Common floor. */
exports.STANDARD_EV_TARGET = 0.65;
function impliedCommonFloorUsd() {
    return (exports.PACKS.standard.priceUsdCents / 100) * exports.STANDARD_EV_TARGET / packExpectedValueMult(exports.PACKS.standard);
}
