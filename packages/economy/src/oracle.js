...
exports.confBps = exports.PythConfidenceError = exports.PYTH_WORST_CASE_AGE_S = exports.PYTH_PUSHER = exports.PYTH_SPONSORED_SHARD_ID = exports.PYTH_SHARD_ID = exports.PYTH_MAX_CONF_BPS = exports.PYTH_SLIPPAGE_BPS = exports.PYTH_MAX_AGE_SECS = exports.PYTH_PROGRAMS = exports.PYTH_FEEDS = void 0;
exports.unitsForCents = unitsForCents;
exports.effectivePythPrice = effectivePythPrice;
exports.maxUnitsWithSlippage = maxUnitsWithSlippage;
exports.pythPriceToUsd = pythPriceToUsd;
exports.pusherCostSolPerMonth = pusherCostSolPerMonth;
exports.PYTH_FEEDS = {
...
exports.PYTH_WORST_CASE_AGE_S = exports.PYTH_PUSHER.timeDifferenceS + exports.PYTH_PUSHER.pushingFrequencyS + 5;
/**
 ...
 */
function unitsForCents(usdCents, price, exponent, decimals) {
    if (price <= 0n)
 ...
 */
function effectivePythPrice(price, conf, maxConfBps = exports.PYTH_MAX_CONF_BPS) {
    if (price <= 0n)
        throw new PythConfidenceError('Pyth price must be positive');
    if (conf * 10000n > price * BigInt(maxConfBps))
        throw new PythConfidenceError(`Pyth confidence ${Number((conf * 10000n) / price) / 100} % exceeds ${maxConfBps / 100} %`);
    const eff = price - conf;
    if (eff <= 0n)
        throw new PythConfidenceError('Pyth confidence swallows the price');
    return eff;
}
class PythConfidenceError extends Error {
}
exports.PythConfidenceError = PythConfidenceError;
/** conf / price in basis points (monitoring / /prices). */
const confBps = (price, conf) => (price <= 0n ? 10_000 : Number((conf * 10000n) / price));
exports.confBps = confBps;
/** Slippage guard passed to buy_pack / pay_service: units × (1 + PYTH_SLIPPAGE_BPS). */
function maxUnitsWithSlippage(units, slippageBps = exports.PYTH_SLIPPAGE_BPS) {
    return (units * BigInt(10_000 + slippageBps)) / 10000n;
}
/** Human price (USD per unit) for display only — never for on-chain amounts. */
function pythPriceToUsd(price, exponent) {
 ...
 */
function pusherCostSolPerMonth(opts = {}) {
    const cuPrice = opts.cuPriceMicroLamports ?? exports.PYTH_PUSHER.computeUnitPriceMicroLamports;
    const cu = opts.cuPerPush ?? 600_000;
    const sigs = opts.signaturesPerPush ?? 4;
    const intervalS = exports.PYTH_PUSHER.timeDifferenceS + exports.PYTH_PUSHER.pushingFrequencyS / 2;
