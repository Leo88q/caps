...
const elf_ts_1 = require("./helpers/elf.ts");
const node_path_1 = require("node:path");
const web3_js_1 = require("@solana/web3.js");
const ROOT = (0, node_path_1.resolve)(import.meta.dirname, '../..');
const DIR = (0, node_path_1.resolve)(ROOT, 'tests/localnet/fixtures');
const RPC = process.env.RPC_URL ?? process.env.ANCHOR_PROVIDER_URL ?? 'https://api.mainnet-beta.solana.com';
const UPGRADEABLE_LOADER = new web3_js_1.PublicKey('BPFLoaderUpgradeab1e11111111111111111111111');
...
async function dump(connection, programId) {
    const program = await connection.getAccountInfo(programId, 'confirmed');
    if (!program)
        throw new Error(`${programId.toBase58()} not found on ${RPC}`);
    if (!program.executable)
        throw new Error(`${programId.toBase58()} is not executable`);
    if (!program.owner.equals(UPGRADEABLE_LOADER))
        return new Uint8Array(program.data); // legacy loaders: data = ELF
    if (program.data.length < 36 || program.data.readUInt32LE(0) !== 2)
        throw new Error(`${programId.toBase58()}: unexpected Program account layout`);
    const programData = new web3_js_1.PublicKey(program.data.subarray(4, 36));
    const pd = await connection.getAccountInfo(programData, 'confirmed');
    if (!pd)
        throw new Error(`ProgramData ${programData.toBase58()} not found`);
    if (pd.data.readUInt32LE(0) !== 3)
        throw new Error(`${programData.toBase58()}: unexpected ProgramData layout`);
...
async function main() {
    ...
    for (const f of FIXTURES) {
        if (only && f.name !== only)
            continue;
        const out = (0, node_path_1.resolve)(DIR, `${f.name}.so`);
        if ((0, node_fs_1.existsSync)(out) && !force) {
            const c = (0, elf_ts_1.checkProgramBinary)(out);
            if (c.ok) {
                console.log(`[fixtures] ${f.name}.so already present (use --force to refetch)`);
                continue;
            }
        ...
        try {
            const elf = useChain
                ? await dump(connection, new web3_js_1.PublicKey(f.id))
                : await downloadRelease(MPL_CORE_RELEASE_URL);
            if (elf.length < 4 || String.fromCharCode(...elf.subarray(1, 4)) !== 'ELF')
                throw new Error('downloaded data does not look like an ELF file');
            (0, node_fs_1.writeFileSync)(out, elf);
            const after = (0, elf_ts_1.checkProgramBinary)(out);
            if (!after.ok)
                throw new Error(`wrote ${elf.length} bytes but the file is not loadable (${after.reason})`);
