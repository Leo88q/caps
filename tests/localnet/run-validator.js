...
const LEDGER = (0, node_path_1.resolve)(ROOT, 'test-ledger');
const OUT = (0, node_path_1.resolve)(ROOT, 'target/localnet');
const PROGRAMS = [
...
const log = (m) => console.log(`[run-validator] ${m}`);
const sh = (cmd, args, opts = {}) => {
    log(`$ ${cmd} ${args.join(' ')}`);
    const r = (0, node_child_process_1.spawnSync)(cmd, args, { stdio: 'inherit', cwd: opts.cwd ?? ROOT, env: { ...process.env, ...opts.env } });
    if (r.error)
        throw new Error(`${cmd}: ${r.error.message}`);
    return r.status ?? 1;
};
const have = (cmd) => (0, node_child_process_1.spawnSync)('sh', ['-c', `command -v ${cmd}`], { stdio: 'ignore' }).status === 0;
/** `solana account --output json`-shaped dump the validator accepts through `--account`. */
function writeAccountDump(path, pubkey, owner, data, lamports = 10_000_000) {
...
async function waitForRpc(child, timeoutMs = 120_000) {
    const t0 = Date.now();
    while (Date.now() - t0 < timeoutMs) {
        if (child.exitCode !== null)
            throw new Error(`solana-test-validator exited with ${child.exitCode} (see test-ledger/validator.log)`);
        try {
            ...
            if (r.ok && (await r.json()).result === 'ok')
                return;
        }
        catch { /* not up yet */ }
...
async function main() {
    for (const bin of ['solana-test-validator', 'solana']) {
        if (!have(bin))
            throw new Error(`${bin} not found in PATH — install the Solana CLI (Anchor.toml pins solana 2.1.x) or use \`npm test\` (LiteSVM, no validator needed)`);
    }
    (0, node_fs_1.mkdirSync)((0, node_path_1.resolve)(ROOT, 'target/deploy'), { recursive: true });
    ...
    if (!process.env.SKIP_BUILD) {
        if (!have('anchor'))
            throw new Error('anchor not found in PATH (or set SKIP_BUILD=1 with prebuilt target/deploy/*.so)');
        if (sh('sh', ['scripts/anchor-build-localnet.sh']) !== 0)
            throw new Error('anchor build failed');
    }
    for (const [, name] of PROGRAMS) {
        const so = (0, node_path_1.resolve)(ROOT, 'target/deploy', `${name}.so`);
        if (!(0, node_fs_1.existsSync)(so))
            throw new Error(`missing ${so}`);
    }
    ...
    if (!(0, node_fs_1.existsSync)(walletPath)) {
        (0, node_fs_1.mkdirSync)((0, node_path_1.dirname)(walletPath), { recursive: true });
        (0, node_fs_1.writeFileSync)(walletPath, JSON.stringify(Array.from(web3_js_1.Keypair.generate().secretKey)));
    ...
    const fixture = (name) => { const f = (0, node_path_1.resolve)(ROOT, 'tests/localnet/fixtures', `${name}.json`); return { path: f, pubkey: JSON.parse((0, node_fs_1.readFileSync)(f, 'utf8')).pubkey }; };
    ...
    writeAccountDump((0, node_path_1.resolve)(OUT, 'admin.json'), admin.publicKey, '11111111111111111111111111111111', Buffer.alloc(0), 1_000_000 * 1_000_000_000);
    // 4. validator
    (0, node_fs_1.rmSync)(LEDGER, { recursive: true, force: true });
    const args = ['--reset', '--quiet', '--ledger', LEDGER, '--rpc-port', String(PORT), '--limit-ledger-size', '50000000'];
    for (const [id, name] of PROGRAMS)
        args.push('--bpf-program', id, (0, node_path_1.resolve)(ROOT, 'target/deploy', `${name}.so`));
    const mplSo = process.env.MPL_CORE_SO ?? ((0, node_fs_1.existsSync)((0, node_path_1.resolve)(ROOT, 'tests/localnet/fixtures/mpl_core.so')) ? (0, node_path_1.resolve)(ROOT, 'tests/localnet/fixtures/mpl_core.so') : undefined);
    const recSo = process.env.PYTH_RECEIVER_SO ?? ((0, node_fs_1.existsSync)((0, node_path_1.resolve)(ROOT, 'tests/localnet/fixtures/pyth_receiver.so')) ? (0, node_path_1.resolve)(ROOT, 'tests/localnet/fixtures/pyth_receiver.so') : undefined);
    if (mplSo)
        args.push('--bpf-program', MPL_CORE, mplSo);
    else
        args.push('--url', CLONE_URL, '--clone-upgradeable-program', MPL_CORE);
    if (recSo)
        args.push('--bpf-program', PYTH_RECEIVER, recSo);
    else {
        if (!args.includes('--url'))
            args.push('--url', CLONE_URL);
        args.push('--clone-upgradeable-program', PYTH_RECEIVER);
    }
    for (const f of [pythSol, pythSkr, { pubkey: admin.publicKey.toBase58(), path: (0, node_path_1.resolve)(OUT, 'admin.json') }])
        args.push('--account', f.pubkey, f.path);
    log(`$ solana-test-validator ${args.join(' ')}`);
    const validator = (0, node_child_process_1.spawn)('solana-test-validator', args, { cwd: ROOT, stdio: ['ignore', 'ignore', 'inherit'] });
    const stop = () => { if (validator.exitCode === null) {
        log('stopping validator');
