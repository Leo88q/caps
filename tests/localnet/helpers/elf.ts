...
export function checkProgramBinary(path: string): ElfCheck {
  ...
  if (buf.subarray(0, 4).toString('latin1') !== '\x7fELF') return { ok: false, reason: `not an ELF (first bytes ${buf.subarray(0, 4).toString('hex')})` };
  if (buf[4] !== 2) return { ok: true }; // not ELF64 — not our toolchain's output; let the loader judge
  const phoff = Number(buf.readBigUInt64LE(0x20));
  const phentsize = buf.readUInt16LE(0x36);
  const phnum = buf.readUInt16LE(0x38);
  if (phnum === 0) return { ok: true }; // headerless blob — nothing structural to check
  if (phentsize < 56) return { ok: false, reason: `bad e_phentsize ${phentsize}` };
  const tableEnd = phoff + phnum * phentsize;
  if (tableEnd > size) return { ok: false, reason: `phdr table (off ${phoff}, ${phnum} x ${phentsize}) exceeds file size ${size} — truncated dump` };
