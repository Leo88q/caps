...
exports.cuIxNameForDisc = exports.CU_IX_NAMES = exports.CU_LOG_PATH = void 0;
exports.cuKeyForIxs = cuKeyForIxs;
exports.recordCu = recordCu;
...
exports.CU_LOG_PATH = 'target/cu-log.jsonl';
...
exports.CU_IX_NAMES = [
    ...
    'claim_root', 'claim_skr_root', 'close_battle_randomness', 'close_expired_claim', 'close_randomness',
...
const discOf = (name) => (0, node_crypto_1.createHash)('sha256').update(`global:${name}`, 'utf8').digest().subarray(0, 8).toString('hex');
const DISC_TO_NAME = new Map(exports.CU_IX_NAMES.map((n) => [discOf(n), n]));
const cuIxNameForDisc = (discHex) => DISC_TO_NAME.get(discHex);
exports.cuIxNameForDisc = cuIxNameForDisc;
const hexOf = (data, len) => Array.from(data.subarray(0, len)).map((b) => b.toString(16).padStart(2, '0')).join('');
/** Native (non-Anchor) ix → short name; `null` = skip (compute budget: noise on every tx). */
function nativeIxName(ix) {
...
function recordCu(label, cu, signature, be, ixs) {
    try {
        (0, node_fs_1.mkdirSync)('target', { recursive: true });
        (0, node_fs_1.appendFileSync)(exports.CU_LOG_PATH, JSON.stringify({ key: cuKeyForIxs(ixs), cu: Number(cu), sig: signature, be, label: label ?? '' }) + '\n');
    }
    catch {
