...
exports.PYTH_EXPO = exports.SKR_USD_PRICE = exports.SOL_USD_PRICE = void 0;
exports.encodePriceUpdateV2 = encodePriceUpdateV2;
exports.unitsForCents = unitsForCents;
exports.postPythPrices = postPythPrices;
exports.refreshPyth = refreshPyth;
exports.forgePriceAccount = forgePriceAccount;
...
exports.SOL_USD_PRICE = 15000000000n;
exports.SKR_USD_PRICE = 1740000n;
exports.PYTH_EXPO = -8;
const DISC = (0, sha256_1.sha256)(new TextEncoder().encode('account:PriceUpdateV2')).slice(0, 8);
function encodePriceUpdateV2(o) {
    const w = new borsh_1.BorshWriter().bytes(DISC).pubkey(web3_js_1.PublicKey.default);
    if (o.partial)
        w.u8(0).u8(5);
    else
        w.u8(1);
    ...
    new DataView(e.buffer).setInt32(0, o.exponent ?? exports.PYTH_EXPO, true);
    w.bytes(e);
 ...
 */
function unitsForCents(cents, f) {
...
const fixtureKey = (name) => web3_js_1.Keypair.fromSeed((0, sha256_1.sha256)(new TextEncoder().encode(`guttercaps/localnet/pyth/${name}`))).publicKey;
const solAccount = web3_js_1.Keypair.generate().publicKey;
const skrAccount = web3_js_1.Keypair.generate().publicKey;
/** Create both fixtures with `publish_time = now` (LiteSVM) or resolve the pre-posted accounts (RPC). */
async function postPythPrices(chain) {
    const prices = {
        sol: { account: chain.kind === 'litesvm' ? solAccount : new web3_js_1.PublicKey(process.env.PYTH_SOL_ACCOUNT ?? fixtureKey('SOL')), feedIdHex: ids_1.PYTH_SOL_USD_FEED_ID_HEX, price: exports.SOL_USD_PRICE, exponent: exports.PYTH_EXPO, decimals: 9, conf: exports.SOL_USD_PRICE / 1000n },
        skr: { account: chain.kind === 'litesvm' ? skrAccount : new web3_js_1.PublicKey(process.env.PYTH_SKR_ACCOUNT ?? fixtureKey('SKR')), feedIdHex: ids_1.PYTH_SKR_USD_FEED_ID_HEX, price: exports.SKR_USD_PRICE, exponent: exports.PYTH_EXPO, decimals: 6, conf: exports.SKR_USD_PRICE / 1000n },
    };
    if (chain.kind === 'litesvm')
        await refreshPyth(chain, prices);
    else {
        for (const f of [prices.sol, prices.skr]) {
            const acc = await chain.getAccount(f.account);
            if (!acc || !acc.owner.equals(ids_1.PYTH_RECEIVER_ID))
                throw new Error(`Pyth fixture ${f.account.toBase58()} missing on the validator — start it with tests/localnet/run-validator.ts (npm run test:validator)`);
        }
    }
    return prices;
}
...
async function refreshPyth(chain, prices, opts = {}) {
    if (chain.kind !== 'litesvm')
        return;
    const now = await chain.now();
    const slot = await chain.slot();
    for (const k of ['sol', 'skr']) {
        const f = prices[k];
        ...
        await chain.setAccount(f.account, { owner: ids_1.PYTH_RECEIVER_ID, data });
    }
}
/** A look-alike price account under a foreign owner (SEC: owner check on price_update). LiteSVM only. */
async function forgePriceAccount(chain, f, owner) {
    const key = web3_js_1.Keypair.generate().publicKey;
