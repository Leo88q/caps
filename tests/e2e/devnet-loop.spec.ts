...
const have = !!EXT && !!SEED;
test.skip(!have, 'T-E-00 needs E2E_PHANTOM_EXTENSION + E2E_DEVNET_MNEMONIC (tests/e2e/README.md)');
...
class Phantom {
  ...
  async init() {
    ...
    if (!sw) sw = await this.ctx.waitForEvent('serviceworker', { timeout: 30_000 });
    this.id = new URL(sw.url()).host;
    if (!this.id) throw new Error('Phantom service worker never registered — is the unpacked build in E2E_PHANTOM_EXTENSION?');
  }

  private popup(): Page {
    const pages = this.ctx.pages().filter((p) => p.url().startsWith(`chrome-extension://${this.id}`));
    if (!pages.length) throw new Error(`no Phantom popup window open (extension ${this.id})`);
    return pages[0];
  }
...
function appWalletMenu(page: Page) {
...
test.describe('T-E-00 · buy → reveal → open → verify, three times', () => {
  test.setTimeout(20 * 60_000);

  test('three standard packs, one wallet, nothing left in the air', async ({ context }) => {
    ...
    await expect(page.locator('.shell-header')).toContainText(/1[1-9A-HJ-NP-Za-km-z]{3,}\.\.[1-9A-HJ-NP-Za-km-z]{4}/);

    // 2 · the app must be looking at devnet too, or the next steps are nonsense
    await expect(page.locator('body')).toContainText(/devnet/i);
    ...
    for (let round = 1; round <= 3; round += 1) {
      ...
      expect(priceBefore.trim().length, `round ${round}: no price rendered before the purchase`).toBeGreaterThan(0);
      ...
      await expect(page).toHaveURL(/\/shop\/opening\//, { timeout: 30_000 });
      ...
      await expect(page.getByRole('button', { name: /reveal|open pack|continue/i }).first())
      ...
      await expect(page.locator('[class*=chip], .chip-grid [role=button]').first()).toBeVisible({ timeout: 120_000 });

      // 8 · deep-link resume: the same pack is still there after a reload (offline-safe opening flow)
      await page.reload();
      await expect(page).toHaveURL(new RegExp(`/shop/opening/${nonce}$`));
      ...
      const sig = await page.evaluate(async (n) => {
        ...
        return (r?.packs ?? []).find((p: { nonce: string }) => String(p.nonce) === String(n))?.signature ?? '';
      }, nonce);
      if (sig) {
        await page.goto(`${APP}/verify/${sig}`);
        await expect(page.getByText(/matches/i).first()).toBeVisible({ timeout: 30_000 });
      }
    }
    ...
    const body = await health.json() as { crank?: { pending?: number; abandoned?: number } };
    expect(body.crank?.abandoned ?? 0, 'crank has abandoned jobs after a three-pack loop').toBe(0);
  });
});
