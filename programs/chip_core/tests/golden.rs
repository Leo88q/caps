...
fn ts_and_rust_expand_agree() {
    let path = concat!(
        ...
        "/../../packages/economy/golden/pack_expand.json"
    );
    let json =
        std::fs::read_to_string(path).expect("run `npm run golden` in packages/economy first");
    ...
    for (n, o) in objs.iter().enumerate() {
        ...
        for (s, &want) in uniform.iter().enumerate() {
            assert_eq!(
                ...
                want,
                "vector {n} uniform slot {s}"
            );
        }
        let eo = effective_odds(def, pity);
        assert_eq!(
            eo.iter().map(|&b| b as i64).collect::<Vec<_>>(),
            odds,
            "vector {n} odds"
        );
        ...
        for (i, p) in pairs.iter().enumerate() {
            ...
            assert_eq!(
                r.collection_idx as i64, p[1],
                "vector {n} slot {i} collection"
            );
        }
        assert!(
            got[pairs.len()..].iter().all(|x| x.is_none()),
            "vector {n} extra slots"
        );
    }
}
