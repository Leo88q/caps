...
- `init_skr_pool` (admin, once) · `fund_skr` (anyone; the treasury multisig runs it weekly with 25 % of SKR pack revenue, 25 % of the treasury part of SKR market fees, 15 % of SKR services revenue — policy in `packages/economy/src/skrRewards.ts`) · `sync_skr_pool` (absorb direct transfers) · `withdraw_skr` / `set_skr_pool` (admin; only unreserved `budget` can leave).
