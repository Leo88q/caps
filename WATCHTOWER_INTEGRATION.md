...
## 2. Эмпирическое исследование утечки памяти ECS (8-12 entities, 10k frames benchmark)

Историческое предположение о деградации памяти («утечка 30% / 1m memref») проверено эмпирическим бенчмарком (`scripts/godot_ecs_benchmark.py`, `tests/godot/ecs_benchmark.gd`, `reports/godot-ecs-benchmark.json`):
- **Статус**: `not_reproduced` (утечка не воспроизводится).
- **Результаты бенчмарка**:
  - Смоделировано кадров: 10 000
  - Создано сущностей: 60 004
  - Корректно освобождено сущностей: 60 004 (100.0%)
  - Активных сущностей по завершении: 0 (0.0% утечки)
  - Диапазон параллельных сущностей: 8–12 (pop-n-shoot loop)
- Архитектурные гарантии:
  ...
  - Полная очистка словарей компонентов в `remove_entity()` исключает накопление слабых ссылок.
