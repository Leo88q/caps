...
6. Пройти `docs/06` §2.3 построчно; особое внимание: `VaultLedger` (`liab_*` ≤ баланс vault), `mint_to_user` (единственный mint-путь), `publish_root` (`budget ≤ slice_budget`), `resolve_battle` (`winner_cg.owner == winner`, дневной cap), `fuse` (`remaining_accounts` layout, `is_free`, эскроу), `open_pack` (пути `revealed`, pity, floors, бандлы), `set_chip_flag` (кто может звать, `expected_owner`).
