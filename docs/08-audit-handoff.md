...
| SEC-B2 | High | числовые query-параметры уходили в SQL без проверки: `?limit=abc/1.5` → 500 `datatype mismatch` на публичном read-эндпоинте, `?limit=-1` → 200 со всей лентой (SQLite: отрицательный `LIMIT` = без лимита, `Math.min(limit, 200)` не работал); `?collection=abc`, `?cursor=abc`, `?sort=bogus`, `?status=stake` молча деградировали | `backend/src/params.ts` (новый), `server.ts`, `queries.ts`, `admin.ts`, `antifraud.ts` | ✅ тесты: `backend/test/params.test.ts` (18, включая свип 18 путей × 16 параметров × 7 значений), статический гейт `tests/security/api-input.test.ts` (6 правил, `security:static` 40/40) |
| SEC-B3 | Medium | `indexMin`/`indexMax`/`sort=index_asc` («Low #») описаны в контракте, отданы в типах и отрисованы в UI, но ничего не делали (в проекции `chips` нет `game_index`, `chipToApi.index` всегда 0) — пользователь видел не ту сортировку/полный список | `backend/openapi.yaml`, `client/src/api/{hooks.ts,schema.d.ts}`, `client/src/features/market/Market.tsx` | ✅ параметры убраны из контракта/UI, сервер отвечает 400 `not_supported`/`bad_sort`; возврат — после проекции `game_index` (SECURITY.md) |
