...
#### SEC-H2 · High (operational) · `admin.rs::set_paused`, `staking::set_paused`, `arena::set_arena` — экстренная пауза заперта за 48-часовым timelock
...
**Тест:** T-L-G03 (pauser может ставить, не может снимать; admin — оба).

#### SEC-H3 · High · `backend/src/server.ts` — нет rate-limiting
...
**Тест:** T-B-40 (превышение → 429), LT-1 (профиль злоупотребления).

#### SEC-M1 · Medium (economy) · `staking::report_burn` никем не вызывается
...
**Тест:** T-L-S05 (guarded_daily растёт после report_burn, не выше cap), T-B-50 (агрегат burn совпадает с суммой событий).

#### SEC-M2 · Medium · Pyth — нет проверки доверительного интервала, SKR-фид тонкий
...
**Статус (после Q7):** возрастной риск снят на уровне ops — свой pusher держит цену ≤ 45 с (алерт > 45 с, критический > 60 с), API отказывает в котировке при < 15 с остатка (`503 price_unavailable`, T-B-39 ✅). Conf-guard on-chain по-прежнему в бэклоге #7; пока `conf_bps` пишется в `oracle_prices` и виден в `GET /prices` для мониторинга.
...
#### SEC-M4 · Medium · backend `auth.ts:46` / `server.ts:38` — SIWS-домен берётся из `x-forwarded-host`
...
**Тест:** T-D-03 (измерить фактическую ренту и стоимость reveal на devnet), T-E-03b.
...
`backend/src` не содержит `open_pack`/`fuse_reveal`/`tick_day`/`thaw` крэнков; единственный «crank» — клиент игрока. Для C3 и для SLA «pending ≤ 20 с» crank обязателен. Спецификация — §4.3.
...
| `set_paused` | admin | — | ✔ | — | — | — | — | — | **H2** нет pauser'а |
...
| `pauser` (новое, **H2**) | Squads 1/3 горячий | `set_paused(true)` | снять паузу может только admin |
...
| `burn_oracle` (**M1**) | то же | `report_burn` | санити-cap |
...
**Бэкенд:** SIWS nonce одноразовый + TTL 5 мин ✔; cookie HttpOnly + `SameSite=None; Secure` в проде ✔; CSRF double-submit ✔; HMAC сессии ✔; подпись проверяется с try/catch (malformed) ✔; события декодируются только от известных program id и дискриминаторов ✔ (тест); идемпотентность ingest ✔; `rebuild` детерминирован ✔; **нет** rate-limit (**H3**), домен из заголовка (**M4**), `confirmed` (**M5**), CORS `*` по умолчанию (**M4**); админ-эндпоинты — 501 (при реализации: allowlist кошельков + подпись SIWS-сообщения с ролью + аудит-лог; kill-switch = вызов `set_paused` через pauser). Секреты не логируются (проверить `pino` redact при внедрении логгера). Prisma-схема — параметризованные запросы ✔ (SQLite-слой использует плейсхолдеры ✔).
...
```
          ...
          ┌─┴──────────────┴─┐
          │ Localnet Anchor  │  T-L  ~60 сценариев, sb_mock + pyth-фикстуры        PR (matrix: programs)
        ┌─┴──────────────────┴─┐
...
**Стратегия зависимостей.** Metaplex Core — клон с mainnet (без оракула, чистая программа) ✔. Pyth receiver — клон программы + **фикстура аккаунта `PriceUpdateV2`** (`solana account --output json` с mainnet, в тестах перезаписываем `publish_time` через `setAccount` bankrun/`--account`), или свой `PriceUpdateV2`-совместимый аккаунт под owner `rec5…` через `--account`. Switchboard On-Demand — **не клонировать**: на localnet нет TEE-оракула, reveal никогда не произойдёт. Пишем `programs/sb_mock` (≈ 80 строк): аккаунт с тем же дискриминатором и layout `RandomnessAccountData` (authority, queue, seed_slothash, seed_slot, oracle, reveal_slot, value, 2 × ebuf), инструкции `init`, `commit` (seed_slot = slot − 1… по флагу «strict»), `reveal(value)` (reveal_slot = текущий слот) и `set_raw(...)` для негативных тестов. Программы собираются с `--features localnet` → `SB_PROGRAM_ID = sb_mock::ID`. Тесты пишем на `anchor-bankrun`/`solana-bankrun` там, где нужен контроль слотов/времени (`warpToSlot`, `setClock`), и на `anchor test` (validator) для полного флоу с реальным клиентским кодом (`client/src/chain/*` импортируется напрямую — это и есть contract-тесты билдерам из `docs/04 §12`).

Замена: `tests/chip-game.ts` (легаси, program id `ChpGame111…`, IDL `client/src/lib/chip_game.idl.json` — файла больше нет) **удалить**; `Anchor.toml [scripts] test` → `yarn ts-mocha -p ./tsconfig.json -t 1000000 tests/localnet/**/*.ts`; `[[test.validator.clone]] SBond…` убрать, добавить `[[test.validator.account]]` для Pyth-фикстуры и `[[test.genesis]]` для `sb_mock.so`.
...
Статус после ответа владельца (сессия после фазы 6): **Q1 — вариант (б) принят и реализован; Q3 — принято; Q4 — «да», уже было в коде; Q5 — оставить 0.5 $CG.** **Сессия 2026-09-15: Q6, Q7, Q8 — решены владельцем и применены (см. ниже).** Открытым остаётся только Q2 (техническая проверка на G-0).
...
| 3 | `STALE_SLOTS = 10 800`, условие `reveal_slot == 0`, authority = PDA + CPI commit, (опц.) CPI reveal (C3) | `economy.rs`, `packs.rs`, `fusion.rs`, `arena`, `client/src/chain/switchboard.ts`, `packFlow.ts`, `fusionFlow.ts`, локали, `docs/02/04`, `programs/README.md`, лендинг | 2 д |
| 4 | `pauser` роль (H2) | `state.rs`, `admin.rs`, staking/arena config, клиент админки | 0.5 д |
| 5 | Rate-limit + SIWS домен/issuedAt + CORS/cookie fail-fast (H3, M4) | `backend/src/server.ts`, `auth.ts`, `config.ts` | 0.5 д |
| 6 | `burn_oracle` + `report_burn` крон (M1) | backend job, `set_oracles`, `emission.rs` (санити-cap) | 0.5 д |
...
| 14 | `programs/sb_mock` + `tests/localnet/*` (60 сценариев) + Pyth-фикстура | новые | 4–5 д |
...
Итого до G-2 ≈ 17–19 инженерных дней (без аудита и soak-периода); #18 добавляет ≈ 1 день на oracle-джоб; #20–21 закрыты в этой сессии, #22 — 0.5 д.
