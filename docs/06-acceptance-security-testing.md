...
| Рефералы (5 % трат, cap 200 $CG) | начисление только по `PackBought` реферала с реальной оплатой; self-referral с того же устройства не окупается (реферал = 4-й+ кошелёк → `device_limit`, сигнал `device_ring`) ✅ | T-B-48, T-B-49 ✅ |
