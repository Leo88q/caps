...
**Бэкенд:** SIWS nonce одноразовый + TTL 5 мин + cap 3/кошелёк ✔; домен из allowlist `SIWS_DOMAINS` + `Issued At` ±5 мин ✔ (**M4** закрыт); cookie HttpOnly + `SameSite=None; Secure` в проде ✔ (fail-fast при `NODE_ENV=production` без `COOKIE_SECURE`/явного CORS/секрета ✔); CSRF double-submit ✔; HMAC сессии ✔; rate-limit по IP/кошельку/сессии + 16 KB тело ✔ (**H3** закрыт); подпись проверяется с try/catch (malformed) ✔; события декодируются только от известных program id и дискриминаторов ✔ (тест); идемпотентность ingest ✔; `rebuild` детерминирован ✔; **остаётся** `confirmed` (**M5**); админ-эндпоинты — 501 (при реализации: allowlist кошельков + подпись SIWS-сообщения с ролью + аудит-лог; kill-switch = вызов `set_paused` через pauser); арена/квесты/стейкинг/fusion — реализованы (#24): очередь и reveal под `requireAuth` + CSRF + rate-limit `arena`, нонсы других игроков скрыты до обоюдного раскрытия, секрет сезона не отдаётся до конца сезона, оракульные ключи только в киперах. Секреты не логируются (проверить `pino` redact при внедрении логгера). Prisma-схема — параметризованные запросы ✔ (SQLite-слой использует плейсхолдеры ✔).
...
| PvP-награды (2/0.5 $CG; 8/д) | ≤ 3 наградных матча/оппонент/д ✅, бот = только участие ✅, форфейт без награды ✅, win-trading граф (бэклог антифрода) | T-B-30..34 |
...
| 9 | `finalized` для денежных проекций (M5) | **сделано**: `backend/src/finality.ts` (реконсилер в `listen`, `finalized_at`, eviction + rebuild, ALERT на consumed), гейт `payment_pending` в `findPayment`, клиент `claimWithRetry`, `/health.finality`, Prisma; T-B-44 ×5 + api + client. Остаток: фильтр `finalized_at IS NOT NULL` в `quests.settleWallet` / reward-oracle (сейчас прогресс из confirmed-проекций; 6-часовой цикл оракула на практике даёт финализацию, но гейт нужен явно — см. #24) | 0.2 д |
...
| 24 | API-заглушки 501 → реальные сервисы: fusion planner, staking read-model, серверная арена (commit–reveal, лиги, боты, форфейты, сезоны), квесты из событий, reward-oracle (Merkle kind 2/3 → `publish_root`), battle-resolver (`resolve_battle`) | **сделано** (эта сессия): `backend/src/{fusion,staking,arena,quests,merkle,reward-oracle,battle-resolver}.ts`, `packages/economy/src/fight.ts`, схема БД, openapi + типы клиента, Arena/MatchReplay UI; T-B-30..38, 51, 52. Остаток: season-oracle payout по итогам сезона (батчер тот же, распределение по `SEASON.payoutBrackets` из `ratings`), минт-путь фишек/бустеров за квесты (сейчас ops), Turnstile на клейм, Redis-очередь при масштабировании, `finalized_at`-фильтр в сеттлменте (#9) | 1 д |
