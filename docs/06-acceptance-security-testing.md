...
| staking | `stake_cg`, `stake_chip`, `claim_chip`, `tick_day`, `publish_root`, `claim_root`, `fund_slice` | `unstake_cg`, `unstake_chip`, `revoke_root`, `report_burn` |
...
```
          ...
          ┌─┴──────────────┴─┐
          │ Localnet (LiteSVM/validator) │ T-L 77 сценариев, sb_mock + pyth-фикстуры   PR (matrix: programs)
        ┌─┴──────────────────┴─┐
...
| Рефералы (5 % трат, cap 200 $CG) — **⚠ начисление не реализовано**: в коде только константы `REFERRAL` (`packages/economy/src/faucets.ts`), `wallets.referrer` из SIWS и детектор `multi_account`; продюсера корня kind 4 (`eventsReserve`, oracle = `season_oracle`) в `reward-oracle.ts` нет → бэклог G-1 | начисление только по `PackBought` реферала с реальной оплатой; self-referral с того же устройства не окупается (реферал = 4-й+ кошелёк → `device_limit`, сигнал `device_ring`) ✅ | T-B-48, T-B-49 ✅ |
...
|---|---|---|---|
| 1 | `owner = SB_PROGRAM_ID` на всех randomness-аккаунтах + cluster features (C1, H1) | **сделано** (не собрано — нет toolchain): `SB_PROGRAM_ID` в `economy.rs` под cluster-feature (`Cargo.toml` ×3, `Anchor.toml`), `#[account(owner = SB_PROGRAM_ID)]`/`require_keys_eq!` на `randomness` в `packs.rs`, `fusion.rs`, `arena/lib.rs`; localnet-спеки `10-packs`/`20-fusion` проверяют отказ на чужом аккаунте | — |
| 2 | Фиксация `value` в `PendingPack` при первом вскрытии (C2) | **сделано** (не собрано — нет toolchain): `PendingPack.revealed/value` (`state.rs`), `open_pack` пишет `value` при первом чтении и дальше использует только его (`packs.rs`); декодеры `client/src/chain/accounts.ts`, backend `chain.ts` | — |
...
| 8 | Fusion fee escrow (M3) | **сделано** (не собрано — нет toolchain): `PendingFusion.fee_escrowed`, эскроу в vault-ATA + `liab_cg`, burn в `fuse_reveal`, возврат в `cancel_stale_fusion`; билдеры client/backend/crank; T-L-F04/F07/F08, crank test | — |
...
| 12 | `VaultLedger` шардирование (§4.2 вывод 1) | **сделано** (не собрано — нет toolchain): `state.rs::VaultLedger` × 4 шарда, `GameConfig` без `liab_*`, `config` read-only в игровых ix, `init_ledger`, `sweep_vault` по 4 шардам, `AccountNotWritable`/`InvalidShard`, декодеры client/backend, admin-разбивка, setup/LUT/sync-check, тесты (см. §4.2) | — |
...
| 16 | CI-workflow (§3.1), Playwright mock/devnet, k6-скрипты, tx-generator | **частично сделано**: `.github/workflows/ci.yml` — jobs `economy` / `client` (+ diff `schema.d.ts` против `openapi.yaml`) / `backend` (+ typecheck `tests/localnet`) / `landing` (rebuild + diff + check + smoke) / `programs` (fmt, clippy, `cargo test`, `anchor build --features localnet` в `solanafoundation/anchor:v0.31.1`, артефакты `.so`/IDL; **advisory** до первой зелёной сборки) / `localnet` (77 сценариев LiteSVM на артефактах, junit) + nightly `e2e-devnet` (read-only, ждёт `DEVNET_RPC_URL`) и `load-smoke` (заглушка до `scripts/load/`). Остаток: Playwright `e2e-mock`, k6-скрипты LT-1, tx-generator | 1.5 д |
...
| 22 | Handoff-пакет для аудитора владельца (Q6): frozen commit, verifiable build, known-issues, канал findings | **сделано**: `docs/08-audit-handoff.md` (скоуп/LOC, команды G-0, таблица findings, что не собрано, принятые риски, ключи devnet, GitHub Security Advisory + SLA 2 р. дня, чек-лист первого дня). Остаток на G-1: вписать frozen commit + тег `audit-v1-<date>` и `solana-verify` хэши | — |
...
| 26 | Рефералы: продюсер корня kind 4 (`referral_rewards` из `PackBought` реферала с `finalized` оплатой, 5 %/cap 200 $CG, `device_limit`), + путь выдачи квест-фишки/бустера (`grant_booster` от `["rewarder"]` PDA, chip-mint через `eventsReserve`) | `backend/src/reward-oracle.ts`, `quests.ts`, `db.ts`, `programs/staking`, `docs/02` | 2 д (G-1) |
...
Итого до G-2 ≈ 9–11 инженерных дней (без аудита и soak-периода); #18 добавляет ≈ 1 день на SKR oracle-джоб; #3 (части 1–2), #14, #20–22, #24 закрыты в этой сессии (для #14 остаётся первый реальный прогон после `anchor build`, для #22 — вписать frozen commit на G-1), #23 — 0.5 д.
