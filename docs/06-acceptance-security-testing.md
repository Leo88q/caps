...
| 9 | `finalized` для денежных проекций (M5) | **сделано**: `backend/src/finality.ts` (реконсилер в `listen`, `finalized_at`, eviction + rebuild, ALERT на consumed), гейт `payment_pending` в `findPayment`, клиент `claimWithRetry`, `/health.finality`, Prisma; T-B-44 ×5 + api + client. Остаток: фильтр `finalized_at IS NOT NULL` в builder'ах квест-/сезон-корней и рефералов, когда они появятся (сейчас 501) | — |
