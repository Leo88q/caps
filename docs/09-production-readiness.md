...
| 1.6 | ⛔ упирается в G-0; G-0 измеряется диагностикой компилятора, а не кодом возврата | 7 051 строка Rust, 28 `#[test]` — зелёной сборки ещё не было, но с прогона 6 красное читается как работа: пин графа снял E0277, и `cargo check` впервые упал **в наших файлах** — 5 диагностик (`pyth.rs:47` E0308: `AccountDeserialize::try_deserialize` просит курсор `&mut &[u8]`, а не срез — тем же срезом были написаны и два теста, поэтому `(lib test)` дал 7 ошибок против 5; `state.rs:123` и `fusion.rs:129` — lifetimes; `fusion.rs:531` — E0716; `admin.rs:87` — E0308 в `seeds`, потому что в `init`-ветке якорного derive массив литералов не аннотирован и `b"collection"` требует `&[u8; 10]` от `&[idx]`). Все пять тронуты в `0cc0c8e`, две правки оказались неполными и доведены до нуля в `3b33237` (прогон 7 сократил 5 до 2 и сам подсказал форму `&'info [AccountInfo<'info>]`; для fusion правильный ответ — взять `&'info`-аккаунт из `ctx.remaining_accounts`, как делают три соседние функции). `market`/`staking`/`arena`/`sb_mock` ещё не компилировались вовсе, так что ноль в chip_core ничего не обещает. Отдельный класс красноты, который ещё не выстрелил: `#[program]` разворачивается в код под `cfg(feature = "solana")`/`custom-heap`/`custom-panic`, rustc вешает `unexpected_cfgs` на строку атрибута (прогон 7: четыре в `sb_mock/src/lib.rs:240`), а `rust-lints` идёт с `-D warnings` → `#[allow(unexpected_cfgs)]` на пяти `#[program]`-модулях; в манифестах (`[lints.rust] check-cfg`) это делать нельзя: cargo понимает там только `level`/`priority`, а SBF-тулчейн образа — cargo 1.79, где и `--check-cfg` в rustc не стабилен (1.80), то есть цена ошибки — не подаленное предупреждение, а нечитающий манифесты гейт. С `0c7e0ac` `anchor build` доходит до installer-проблемы образа (agave переименовал `solana-install` → `agave-install`, а у контейнера нет `~/.config/solana/install/config.yml`) — обе закрыты шимом в `scripts/ci-anchor-build.sh`, так что следующий прогон впервые покажет **реальный compile-pass**. Всё красное, что он покажет, — работа, а не конфиг |
