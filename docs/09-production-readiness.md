...
| 1.4 | 🟡 автоматизировано, ждёт первого зелёного прогона | `Cargo.lock` не мог появиться «вручную»: песочница без `cargo`, а сгенерировать его должен тот же resolver, что и собирает. Это делает `lockfile.yml` (gate: лока есть/`--locked` не сходится → resolve+pin в `solanafoundation/anchor:v0.31.1` → `cargo check` как доказательство → коммит в ветку / PR в main). `programs` по-прежнему генерирует лок прогонно и требует `git diff --exit-code`, так что после коммита он не сможет тихо устареть |
...
| 1.6 | ⛔ упирается в G-0, и G-0 теперь измерим | 7 051 строка Rust, 28 `#[test]` — ни разу не скомпилированы (единственный красный шаг `clippy` на runs 40–42 — это E0277 графа из §1.4, а не стиль; unit-тесты, `anchor build`, проверки keypair/IDL под ним пропускались). С `0c7e0ac` `anchor build` доходит до installer-проблемы образа (agave переименовал `solana-install` → `agave-install`, а у контейнера нет `~/.config/solana/install/config.yml`) — обе закрыты шимом в `scripts/ci-anchor-build.sh`, так что следующий прогон впервые покажет **реальный compile-pass**. Всё красное, что он покажет, — работа, а не конфиг |
