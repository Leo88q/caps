...
|---|---|
| Любой прогон в браузере (Playwright mock-тир, Lighthouse, axe в браузере) | `npx playwright install chromium` → «Download failure, code=1»: CDN недоступен. Специки исполняются **только в CI** (job `e2e`); локально им доступна лишь проверка типов. Отсюда правило: строка «8 тестов» в §5.4 — про CI-прогон, а не про мой |
