...
Отсюда три правила, которые раньше были толькоявно. (1) Когда блокирует **наш** диапазон, лок его не
...
= false` сохранён. (2) Понятие «новая читаемая версия», на котором живёт обход, для держателей **недостаточно** (1.8.6 читается, но несёт то же самое):
`blake3 1.8.7 → 1.8.6` читается сам и по-прежнему тащит `digest 0.11`, поэтому вручную иногда надо прыгать ниже
(1.5.5) —curated-список затем и остаётся, а не дублирует обход. (3) Если на строке нет ничего читаемого,
