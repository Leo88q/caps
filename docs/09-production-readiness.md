...
> исправлены по месту, правило ради них не ослаблялось. План с тех пор исполнен частично,
...
rust (CI)    `cargo clippy --workspace --all-targets -- -D warnings` + `cargo test --workspace` — зелёные
             на 9303e2e (run 61). Это первый исполненный прогон тестов в репозитории: 28 `#[test]` в пяти
             крейтах, 10 165 строк src, и первый зелёный `-D warnings`; ниже по §1.4 — почему тесты зелёные,
             а G-0 всё ещё нет
...
|---|---|---|---|
| G-0 Compile | `anchor build` + `cargo test` зелёные, IDL, `keys sync` | 🟡 **33 %** — `cargo test --workspace` и `clippy -D warnings` зелёные с `9303e2e` (run 61, первый прогон тестов в истории репозитория); `.so`/IDL — нет | нет тулчейна/сети в среде разработки; 4 подтверждённых риска в `Cargo.toml` (§1) |
...
эскалации на середине. Фикстура (`/tmp/fa6` + stateful fake cargo) проверяет обе ветки: принятое понижение
держателя на уровень выше и отказ с корректной эскалацией. Попутно фикстура нашла живой баг — `rm -f "$upd"`
в ветке нечитаемого индекса ссылался на переменную, которая там ещё не создана, и под `set -u` обрывал скрипт
...
«индекс не прочитан про crate `sbf-autopin:`». Самопроверка: `/tmp/fc2` (stateful fake cargo) + `/tmp/fa6`
(раскладка с discoverable SBF-cargo), четыре сценария A–D, каждый сверяет текст, а не только код возврата.

Остаётся развилка, которую скрипт закрывать не должен, потому что она про развёртывание, а не про лок:
либо дописывать в `SBFPINS` каскад RustCrypto (и он будет пополняться — это вечная политика), либо поднять
`solana_version` в `Anchor.toml` так, чтобы платформенный cargo читал edition2024 (в 2.1.0 это 1.79; у более
свежих Agave платформы новее), — тогда весь класс исчезает, но меняется тулчейн, которым собирается `.so`,
а значит и то, что увидят ревью и `solana-verify` при сверке воспроизводимой сборки. Второе — решение про
кластер и про аудит, не про CI; здесь оно помечено как открытое, а не сделанное.
