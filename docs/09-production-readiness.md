...
Ни один ончейн-артефакт этого проекта ни разу не существовал. `Cargo.lock` лежит в ветке с `7bcc809` (350
пакетов, произведён `lockfile.yml` и прочитан SBF-cargo 1.79 — иначе бот не коммитит), зелёные прогоны CI в
истории появились (`landing`, `economy`, `lockfile.yml` run 31), но `target/deploy/*.so` нет, IDL нет,
`anchor build` ни разу не доходил до компиляции программ, задеплоенных программ нет.
