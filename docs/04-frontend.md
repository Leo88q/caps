...
STALE ──► если >300 слотов без value: кнопка «Вернуть деньги» → cancel_stale_pack (100 % из vault)
```
...
- Ошибки Switchboard/сети не теряют деньги: любой «висящий» пак виден на Home и в `/shop/opening/:nonce` с кнопкой refund после 300 слотов.
