...
| кипер `reward-oracle` (`npm run backend:reward-oracle`) | `backend/src/reward-oracle.ts` + `merkle.ts` | раз в 6 ч: сеттлмент квестов активных кошельков + **завершённых сезонов** (`arena.settleSeason`: ≥ 10 матчей без форфейта, ранжирование по рейтингу, `seasonPayoutByRank` по брекетам 15/20/25/25/15 % с roll-up пустых полос и масштабированием пула на n/1000 участников) → батчи kind 2 (квесты) и kind 3 (PvP-матчи + сезонные выплаты) → дерево, побайтно равное `staking::verify_proof` (golden-вектор общий с клиентом и Rust) → `publish_root(kind, epoch, root, budget)` ключом `QUEST_ORACLE_KEYPAIR` / `SEASON_ORACLE_KEYPAIR`; листья + пруфы отдаёт `/quests/claims` (`claimableAt = RootPublished + 1 ч`); ничего не минтит — бюджет ограничен `slice_budget`, накопленным `tick_day` |
