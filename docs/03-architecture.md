...
`501 not_implemented` в API больше нет: `/admin/*` реализован в `backend/src/admin.ts` (§3.5). Бустеры за квесты доставляются on-chain (item-корни kind 8 → `claim_item_root` → CPI `grant_booster`, #27). Награды **фишками** за квесты (стрик, «все weekly», вехи) в v1 без минт-пути: фиксируются в `quest_completions.reward_chip` для админ-минта (backlog #28). См. `backend/README.md`.
