...
- [x] 27-я партия (аудит 2026-09-25): пары-близнецы 04-3/05-1/07-1 усилены по референсу предыдущего тира («следующий слой» читается: RMS 5.1→30.9, 9.0→57.6, 7.8→47.7; старые мастера — raw/{04-3,05-1,07-1}_v2.png); тональный лифт глухих кадров 03-6/03-7/10-6/10-7 (σ130 26–32 → 32–37, обод не тронут, старые — raw/{…}_v2.png); экспорты пайплайна docs/07 §4 выпущены (exports/cdn/{game,nft,og,m} + 48 видео-лупов tiers 6–8)
...
- [ ] Финальный QA: программные метрики сняты по каждому кадру (2048², Ø 87,5 %, альфа 0/255, тёмные лица — журналы партий в nft_tracker.md); осталось: визуальное подтверждение владельца (экспорты §4 выпущены — exports/cdn/, метрики в reports/)
