...
3. ✅ **РЕШЕНО владельцем (25.09): перекрашивать кадры.** 01/02/06 перекрашены под лор дуотон-скриптом `scripts/art-recolor-districts.py` (оригиналы `raw/{…}_v3.png`, ободы сохранены); 03/05 уже в лоре (§8, партии 21/23) — правка не требуется.
...
- [x] Район 01: 9/9 перегенерировано (19-я партия: вкл. 01-2 портфель и 01-8 лужа; превью preview_district01.png)
- [x] Район 02: 9/9 готово (20–21 партии; 02-1 шлейф-эхо доведён в 21-й) · [x] Район 03: 9/9 перегенерировано (21-я партия, маджента по лору, пары — генерацией по референсу; превью preview_district03.png) · [x] Район 04: 9/9 готово (22-я партия: 8 кадров + 04-5 из 18-й; оранжевая нить-подпись + #9AD9FF, пары референсами; превью preview_district04.png; инструмент make_master_robust.py) · [x] Район 05: 9/9 готово (23-я партия: 8 кадров + 05-5 из 18-й; acid по лору, кабель от фонаря как сквозной мотив, 05-4 сгенерирован референсом готового 05-5; превью preview_district05.png) · [x] Район 06: 9/9 готово (24-я партия: 8 кадров + 06-5 из 18-й; cyan по лору, эмаль-стиль, все четыре пары закрыты референсами, кот с одним глазом, серые птенцы; превью preview_district06.png) · [x] Район 07: 9/9 готово (25-я партия: 8 кадров + 07-2 из 18-й; маджента по лору, очки только цифрами, 07-3 сгенерирован референсом готового 07-2 — решение №5 реализовано; превью preview_district07.png)
- [x] Район 10: 9/9 готово (26-я партия: 4 регена 10-0/1/6/8 с правками §5.4/§5.5 — 9 пинов, абстрактный Gutter City, петля на 01 в 10-8; + 5 новых 10-2/3/4/5/7 по промтам §4; рваная бумага, acid по лору; превью preview_district10.png) — **ВСЕЛЕННАЯ 72/72**
- [x] Нормализация: рим-лестница §3 применена при генерации (все партии 18–26, материал обода = тир в каждом промте); [ ] связки пар — финальная сверка владельцем по превью районов (пары построены генерацией по референсу)
- [x] Ретушь-сессия 2026-09-25 (отчёт ASSET-INVENTORY-2026-09-25.md, 5-я итерация): контраст тира 6 поднят γ-лифтом (все 8 районов, двойной проход 03-6/03-7/04-6/10-6); обод Diamond перекрашен в хром-голограмму (циан, sat 39–42); пары-близнецы перегенерированы по референсу — 04-3 «Triple Pass», 05-1 «Encore», 07-1 «Continue? 9», 07-7 «Repeat» (RMS пар 5.1–10.9 → 96.9–100.0). Скрипты: art-retouch-p2.py, asset-contact-sheets.py
- [x] Экспорты пайплайна docs/07 §4 — ✅ готово: `npm run assets:pipeline -- --animation` (exports/cdn/{game,nft,og,m}, 216 webp в бюджетах, 72 карточки, 72 OG, 72 metadata JSON, 48 видео-лупов 6–8 тиров)
- [ ] Финальный QA: программные метрики сняты по каждому кадру (2048², Ø 87,5 %, альфа 0/255 — чек-лист 72/72 ✅, тест 130/56 px ✅ по контакт-листам); осталось: визуальное подтверждение владельца
