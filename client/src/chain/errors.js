...
exports.describeProgramError = describeProgramError;
exports.humanizeTxError = humanizeTxError;
...
const TABLES = [
    { id: ids_1.CHIP_CORE_ID.toBase58(), table: CHIP_CORE, name: 'chip_core' },
    { id: ids_1.MARKET_ID.toBase58(), table: MARKET, name: 'market' },
    { id: ids_1.STAKING_ID.toBase58(), table: STAKING, name: 'staking' },
    { id: ids_1.ARENA_ID.toBase58(), table: ARENA, name: 'arena' },
];
function describeProgramError(code, programId) {
    if (code < 6000)
        return ANCHOR[code];
    const idx = code - 6000;
    if (programId) {
        ...
        if (t)
            return t.table[idx] ? `${t.name}: ${t.table[idx]}` : `${t.name}: error ${code}`;
    }
...
function humanizeTxError(err) {
    const msg = String(err?.message ?? err ?? 'Unknown error');
    if (/User rejected|rejected the request|declined/i.test(msg))
        return 'Signature rejected in wallet';
    if (/insufficient lamports|insufficient funds|0x1$/i.test(msg))
        return 'Insufficient SOL for this transaction';
    ...
    if (/Transaction too large/i.test(msg))
        return 'Transaction too large (use a smaller bundle)';
    const custom = (0, anchor_1.parseCustomError)(err);
    if (custom) {
