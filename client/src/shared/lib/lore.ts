// Single source of truth for GUTTERCAPS lore, mirrored 1:1 from the
// marketing site's COLLECTIONS array (guttercaps-landing.html) so the game
// and the site never drift apart on names/history. `symbol` is new here —
// it's the on-chain Collection.symbol (max 16 ASCII bytes), which the site
// doesn't need since it isn't touching the contract.
