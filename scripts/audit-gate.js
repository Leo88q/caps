...
const ghsaOf = (a) => { const m = /GHSA-[a-z0-9]{4}-[a-z0-9]{4}-[a-z0-9]{4}/.exec(a.url ?? ''); return m ? m[0] : `npm:${a.source}`; };
/** Pure evaluation of a report against the allowlist, so the selftest needs no network. */
function evaluate(report, accepted = exports.ACCEPTED, today = new Date().toISOString().slice(0, 10), minSeverity = 'high') {
    const failures = [];
    const notes = [];
    ...
    for (const a of accepted)
        if (a.until < today)
            failures.push(`accepted entry ${a.id} (${a.pkg}) expired on ${a.until} — re-evaluate it, then move the date or remove the entry`);
    ...
    const rootsOf = (name, stack = []) => {
        ...
        for (const via of v.via) {
            if (typeof via === 'string') {
                if (!stack.includes(via))
                    for (const id of rootsOf(via, [...stack, name]))
                        out.add(id);
            }
            else
                out.add(`${ghsaOf(via)}|${via.name}|${via.title}|${via.severity}`);
        }
        return out;
    };
    for (const [name, v] of Object.entries(vulns).sort()) {
        if ((SEVERITY_RANK[v.severity] ?? 0) < (SEVERITY_RANK[minSeverity] ?? 3))
            continue;
        const roots = [...rootsOf(name)];
        const uncovered = roots.filter((r) => !acceptedIds.has(r.split('|')[0]));
        for (const r of roots)
            if (acceptedIds.has(r.split('|')[0]))
                used.add(r.split('|')[0]);
        if (roots.length === 0) {
            failures.push(`${name} (${v.severity}) has no advisory in its chain — npm changed the report shape; update audit-gate.ts`);
            continue;
        }
        if (uncovered.length) {
            for (const r of uncovered) {
                const [id, pkg, title, sev] = r.split('|');
                failures.push(`${name} ${v.range} (${v.severity}) ← ${pkg}: ${title} [${id}, ${sev}]${v.fixAvailable ? typeof v.fixAvailable === 'object' ? ` — fix: ${v.fixAvailable.name}@${v.fixAvailable.version}${v.fixAvailable.isSemVerMajor ? ' (major)' : ''}` : ' — `npm audit fix`' : ' — no fix available'}`);
            }
        }
        else
            notes.push(`${name} ${v.range} (${v.severity}) — covered by ${[...new Set(roots.map((r) => r.split('|')[0]))].join(', ')}`);
    }
...
function runAudit() {
    const r = (0, node_child_process_1.spawnSync)('npm', ['audit', '--omit=dev', '--json'], { encoding: 'utf8', maxBuffer: 64 * 1024 * 1024 });
    ...
    if (start < 0)
        throw new Error(`npm audit produced no JSON (exit ${r.status}): ${(r.stderr ?? '').trim().split('\n').slice(-3).join(' | ')}`);
    const report = JSON.parse(text.slice(start));
    if (report.error)
        throw new Error(`npm audit error ${report.error.code ?? ''}: ${report.error.summary ?? ''}`);
    return report;
}
// --------------------------------------------------------------------------- selftest
const adv = (id, name, severity, title = `${name} problem`) => ({ source: 1, name, title, url: `https://github.com/advisories/${id}`, severity, range: '*' });
const canned = () => ({
...
const cases = [
    {
        ...
        run: () => {
            const r = evaluate(canned(), exports.ACCEPTED, '2026-09-23');
            const p = [];
            if (!r.ok)
                p.push(`expected ok: ${r.failures.join(' | ')}`);
            if (r.notes.length !== 3)
                p.push(`expected 3 covered notes, got ${r.notes.length}: ${r.notes.join(' | ')}`);
            if (r.stale.length)
                p.push(`unexpected stale: ${r.stale.join(' | ')}`);
            if (!r.accepted.includes('GHSA-3gc7-fjrx-p6mg'))
                p.push('the used id must be reported');
            return p;
        },
    },
    {
        ...
        run: () => {
            ...
            const r = evaluate(rep, exports.ACCEPTED, '2026-09-23');
            const p = [];
            if (r.ok)
                p.push('expected failure');
            if (!r.failures.some((f) => f.startsWith('fresh-lib') && f.includes('GHSA-zzzz-zzzz-zzzz') && f.includes('RCE via parse') && f.includes('top@3.0.0 (major)')))
                p.push(`fresh-lib failure malformed: ${r.failures.join(' | ')}`);
            if (!r.failures.some((f) => f.startsWith('top ') && f.includes('GHSA-zzzz-zzzz-zzzz')))
                p.push(`the dependant must be attributed to the same root: ${r.failures.join(' | ')}`);
            return p;
        },
    },
    {
        ...
        run: () => {
            const p = [];
            const expired = evaluate(canned(), [{ ...exports.ACCEPTED[0], until: '2026-01-01' }], '2026-09-23');
            if (expired.ok || !expired.failures.some((f) => f.includes('expired on 2026-01-01')))
                p.push(`expected expiry failure: ${expired.failures.join(' | ')}`);
            const stale = evaluate(canned(), [...exports.ACCEPTED, { id: 'GHSA-gone-gone-gone', pkg: 'ghost', until: '2099-01-01', why: 'x' }], '2026-09-23');
            if (!stale.ok)
                p.push(`stale must not fail the gate: ${stale.failures.join(' | ')}`);
            if (stale.stale.length !== 1 || !stale.stale[0].includes('GHSA-gone-gone-gone'))
                p.push(`expected one stale entry: ${stale.stale.join(' | ')}`);
            return p;
        },
    },
    {
...
function main(argv) {
    ...
    for (const s of r.stale)
        console.log(process.env.GITHUB_ACTIONS ? `::warning::audit-gate: ${s}` : `  ! ${s}`);
    for (const f of r.failures)
        console.error(process.env.GITHUB_ACTIONS ? `::error::audit-gate: ${f}` : `  ✗ ${f}`);
