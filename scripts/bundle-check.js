...
const ROOT = node_path_1.default.resolve(node_path_1.default.dirname((0, node_url_1.fileURLToPath)(import.meta.url)), '..');
const DIST = node_path_1.default.join(ROOT, 'client/dist');
const BUDGET_KB = Number(process.env.BUNDLE_BUDGET_KB ?? 350);
if (!(0, node_fs_1.existsSync)(node_path_1.default.join(DIST, 'index.html'))) {
    console.error('client/dist/index.html missing — run `npm --prefix client run build` first');
    process.exit(1);
}
const html = (0, node_fs_1.readFileSync)(node_path_1.default.join(DIST, 'index.html'), 'utf8');
// Exactly the two tags a renderer must have before first paint, in whatever order Vite emits them.
const urls = new Set();
for (const m of html.matchAll(/<link rel="(?:modulepreload|stylesheet)"[^>]*href="([^"]+)"/g))
    urls.add(m[1]);
...
if (external.length) {
    console.error(`::error::built index.html references ${external.length} off-origin URL(s):\n  ${external.join('\n  ')}\n` +
...
const assetsDir = node_path_1.default.join(DIST, 'assets');
const hits = [];
for (const f of (0, node_fs_1.existsSync)(assetsDir) ? (0, node_fs_1.readdirSync)(assetsDir).filter((n) => /\.(css|js)$/.test(n)) : []) {
    const src = (0, node_fs_1.readFileSync)(node_path_1.default.join(assetsDir, f), 'utf8');
    ...
    if (f.endsWith('.css')) {
        for (const m of src.matchAll(/(?:@import\s+)?url\(\s*['"]?https?:\/\/[^)'"]+/g))
            hits.push(`${f}: ${m[0].slice(0, 90)}`);
    }
    else {
        for (const m of src.matchAll(/<link\b[^>]*?(?:href|src)=["']https?:\/\/[^"']*/g))
            hits.push(`${f}: ${m[0].slice(0, 90)}`);
    }
}
if (hits.length) {
    console.error(`::error::built assets reference ${hits.length} off-origin resource(s):\n  ${hits.join('\n  ')}\n` +
...
for (const u of [...urls].sort()) {
    const file = node_path_1.default.join(DIST, u.replace(/^\/+/, ''));
    if (!(0, node_fs_1.existsSync)(file)) {
        console.error(`index.html references a missing file: ${u}`);
        process.exit(1);
    }
    ...
    const gz = (0, node_zlib_1.gzipSync)((0, node_fs_1.readFileSync)(file)).length;
    total += gz;
...
for (const r of rows)
    console.log(r);
if (over) {
    console.error(`::error::critical-path bundle is ${kb.toFixed(1)} KB > ${BUDGET_KB} KB. Common cause: a heavy ` +
