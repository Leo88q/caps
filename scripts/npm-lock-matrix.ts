...
type Lock = { lockfileVersion?: number; packages?: Record<string, { optionalDependencies?: Record<string, string> }> };
type Finding = { parent: string; name: string; range: string };
 ...
 */
function missingOptional(packages: Record<string, { optionalDependencies?: Record<string, string> }>): Finding[] {
  ...
  for (const [parent, entry] of Object.entries(packages)) {
    for (const [name, range] of Object.entries(entry.optionalDependencies ?? {})) {
      ...
      if (!candidates.some((c) => c in packages)) findings.push({ parent: parent || '(root)', name, range });
    }
  }
  return findings;
}

function report(findings: Finding[], lockPath: string, checked: number): number {
  if (!findings.length) {
  ...
  for (const f of findings.slice(0, 12)) console.error(`    ${f.parent} declares ${f.name}@${f.range}`);
...
function load(path: string): Lock {
...
function selftest(): void {
  const cases: Array<[string, Record<string, { optionalDependencies?: Record<string, string> }>, number]> = [
...
if (argv.includes('--selftest')) {
  ...
  process.exit(check(lockArg >= 0 ? resolve(argv[lockArg + 1] ?? '') : resolve(root, 'package-lock.json')));
}
