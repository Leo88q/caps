...
const lore_ts_1 = require("../client/src/shared/lib/lore.ts");
const tokenomics_ts_1 = require("../packages/economy/src/tokenomics.ts");
const verify_deploy_ts_1 = require("./verify-deploy.ts");
...
const SQUADS_TREASURY = new web3_js_1.PublicKey('HPMr5r9sS5ApWsPNJytZRLbm2jz1veFxTn1wepjAhtho');
const REAL_USDC = new web3_js_1.PublicKey('EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v');
const DEVNET_USDC = new web3_js_1.PublicKey('4zMMC9srt5Ri5X14GAgXhaHii3GnPAEERYPJgZJDncDU');
const REAL_SKR = new web3_js_1.PublicKey('SKRbvo6Gf7GondiT3BbTfuRDPqLWei4j2Qy2NPGZhW3');
const PYTH_SOL = new web3_js_1.PublicKey(process.env.PYTH_SOL_ACCOUNT ?? 'ELp9x5sFxGJ7zTurykU2p6A9nKDx72b3xzPxfsB5S8GB');
const PYTH_SKR = new web3_js_1.PublicKey(process.env.PYTH_SKR_ACCOUNT ?? '9bCSdQVWckgKipe4G3G66aYU9yq2ZdDn8kRPZB9Nihbc');
/** lore element per collection index (client/src/shared/lib/rarity.ts ELEMENT_OF_COLLECTION) → on-chain u8 */
const ELEMENT_OF_COLLECTION = ['shadow', 'wheels', 'steel', 'wheels', 'noise', 'shadow', 'noise', 'wheels', 'paint', 'paint'];
const ELEMENT_INDEX = { paint: 0, steel: 1, wheels: 2, noise: 3, shadow: 4 };
...
const disc = (name) => Buffer.from((0, sha256_1.sha256)(`global:${name}`).subarray(0, 8));
...
class W {
    ...
    u64(v) { const b = Buffer.alloc(8); b.writeBigUInt64LE(v); this.parts.push(b); return this; }
    i64(v) { const b = Buffer.alloc(8); b.writeBigInt64LE(v); this.parts.push(b); return this; }
    pubkey(k) { this.parts.push(Buffer.from(k.toBytes())); return this; }
    string(s) { const e = Buffer.from(s, 'utf8'); const l = Buffer.alloc(4); l.writeUInt32LE(e.length); this.parts.push(l, e); return this; }
    bytes() { return Buffer.concat(this.parts); }
}
const ix = (programId, name, keys, args = Buffer.alloc(0)) => new web3_js_1.TransactionInstruction({ programId, keys, data: Buffer.concat([disc(name), args]) });
const pda = (seeds, program) => web3_js_1.PublicKey.findProgramAddressSync(seeds, program)[0];
const ata = (mint, owner) => (0, spl_token_1.getAssociatedTokenAddressSync)(mint, owner, true);
...
const ledgerPda = (shard) => pda([Buffer.from('ledger'), Buffer.from([shard])], CHIP_CORE);
const collectionMetaPda = (i) => pda([Buffer.from('collection'), Buffer.from([i])], CHIP_CORE);
...
const envKey = (name, dflt) => (process.env[name] ? new web3_js_1.PublicKey(process.env[name]) : dflt);
async function send(conn, payer, ixs, label, extra = []) {
    if (DRY_RUN) {
        console.log(`[dry-run] ${label}: ${ixs.length} ix`);
        return 'dry-run';
    }
    const tx = new web3_js_1.Transaction().add(...ixs);
    const sig = await (0, web3_js_1.sendAndConfirmTransaction)(conn, tx, [payer, ...extra], { commitment: 'confirmed' });
    console.log(`  ✓ ${label}: ${sig}`);
    return sig;
}
const exists = async (conn, key) => (await conn.getAccountInfo(key, 'confirmed')) !== null;
// ---------------------------------------------------------------- GameConfig readers (offsets: 8 disc + admin, pending, treasury, buyback, cg, usdc, skr, staking, pythSol, pythSkr)
function readConfig(data) {
    const pk = (o) => new web3_js_1.PublicKey(data.subarray(o, o + 32));
...
// ---------------------------------------------------------------- steps
async function stepMints(conn, wallet) {
    const out = { cg: process.env.CG_MINT ? new web3_js_1.PublicKey(process.env.CG_MINT) : undefined, usdc: envKey('USDC_MINT', MAINNET ? REAL_USDC : DEVNET_USDC), skr: process.env.SKR_MINT ? new web3_js_1.PublicKey(process.env.SKR_MINT) : MAINNET ? REAL_SKR : undefined };
    ...
    const create = async (label) => {
        ...
        await send(conn, wallet, [
            web3_js_1.SystemProgram.createAccount({ fromPubkey: wallet.publicKey, newAccountPubkey: mint.publicKey, lamports: await conn.getMinimumBalanceForRentExemption(spl_token_1.MINT_SIZE), space: spl_token_1.MINT_SIZE, programId: spl_token_1.TOKEN_PROGRAM_ID }),
            (0, spl_token_1.createInitializeMint2Instruction)(mint.publicKey, 6, wallet.publicKey, null),
        ], `create ${label} mint ${mint.publicKey.toBase58()}`, [mint]);
        return mint.publicKey;
    };
    if (!out.cg) {
        if (MAINNET)
            throw new Error('mainnet: CG_MINT is required (created by the treasury multisig with the wallet as temporary mint authority)');
        out.cg = await create('$CG');
    }
    if (!out.skr)
        out.skr = await create('SKR (devnet stand-in)');
    return out;
}
async function stepInitialize(conn, wallet, mints, treasury, buyback) {
    if (await exists(conn, configPda)) {
        console.log('  initialize: config exists — skip');
        return;
    }
    ...
    await send(conn, wallet, [ix(CHIP_CORE, 'initialize', [signer(wallet.publicKey), rw(configPda), rw(vaultPda), ro(web3_js_1.SystemProgram.programId)], args)], 'initialize');
}
async function stepLedgers(conn, wallet) {
    const missing = [];
    for (let i = 0; i < LEDGER_SHARDS; i++)
        if (!(await exists(conn, ledgerPda(i))))
            missing.push(i);
    if (!missing.length) {
        console.log(`  ledgers: all ${LEDGER_SHARDS} shards exist — skip`);
        return;
    }
    await send(conn, wallet, missing.map((i) => ix(CHIP_CORE, 'init_ledger', [signer(wallet.publicKey), rw(ledgerPda(i)), ro(web3_js_1.SystemProgram.programId)], new W().u8(i).bytes())), `init_ledger ${missing.join(',')}`);
}
async function stepCollections(conn, wallet) {
    const cfg = await conn.getAccountInfo(configPda, 'confirmed');
    if (!cfg)
        throw new Error('run `initialize` first');
    let created = readConfig(cfg.data).collectionsCreated;
    for (let i = created; i < lore_ts_1.COLLECTIONS.length; i++) {
        const c = lore_ts_1.COLLECTIONS[i];
        ...
        await send(conn, wallet, [ix(CHIP_CORE, 'create_collection', [signer(wallet.publicKey), rw(configPda), rw(collectionMetaPda(i)), signer(core.publicKey), ro(MPL_CORE), ro(web3_js_1.SystemProgram.programId)], args)], `create_collection ${i} ${c.symbol} (core ${core.publicKey.toBase58()})`, [core]);
        created++;
    }
    console.log(`  collections: ${created}/${lore_ts_1.COLLECTIONS.length}`);
}
async function stepAtas(conn, wallet, mints, treasury, buyback) {
    const ixs = [];
    for (const m of [mints.cg, mints.usdc, mints.skr]) {
        for (const owner of [vaultPda, treasury, buyback]) {
            if (!(await exists(conn, ata(m, owner))))
                ixs.push((0, spl_token_1.createAssociatedTokenAccountIdempotentInstruction)(wallet.publicKey, ata(m, owner), owner, m));
        }
    }
    if (ixs.length === 0) {
        console.log('  atas: all present — skip');
        return;
    }
    await send(conn, wallet, ixs, `create ${ixs.length} token accounts (vault / treasury / buyback)`);
}
async function stepEmission(conn, wallet, cg) {
    if (await exists(conn, emissionPda)) {
        console.log('  emission: exists — skip');
        return;
    }
    ...
    const split = [tokenomics_ts_1.EMISSION_SPLIT.chipStaking, tokenomics_ts_1.EMISSION_SPLIT.tokenStaking, tokenomics_ts_1.EMISSION_SPLIT.quests, tokenomics_ts_1.EMISSION_SPLIT.pvpSeason, tokenomics_ts_1.EMISSION_SPLIT.eventsReserve].map((p) => p * 100);
    ...
    await send(conn, wallet, [ix(STAKING, 'init_emission', [signer(wallet.publicKey), rw(emissionPda), rw(tokenPoolPda), rw(chipPoolPda), rw(cg), ro(spl_token_1.TOKEN_PROGRAM_ID), ro(web3_js_1.SystemProgram.programId)], w.bytes())], 'init_emission ($CG mint authority → emission PDA)');
    for (const [k, v] of Object.entries(oracles))
        if (v.equals(wallet.publicKey))
            console.warn(`  !! ${k} oracle = deployer wallet — set ${k.toUpperCase()}_ORACLE and call set_oracles before G-1`);
...
async function stepBurnOracle(conn, wallet) {
    const key = process.env.BURN_ORACLE;
    if (!key) {
        console.log('  burn-oracle: BURN_ORACLE not set — skip');
        return;
    }
    const burn = new web3_js_1.PublicKey(key);
...
async function stepArena(conn, wallet, cg, treasury) {
    if (await exists(conn, arenaConfigPda)) {
        console.log('  arena: exists — skip');
        return;
    }
    ...
    const cap = BigInt(process.env.ORACLE_DAILY_CAP_CG ?? tokenomics_ts_1.ARENA_ORACLE_DAILY_CAP_DEFAULT_CG) * MICRO;
    const seasonPool = ata(cg, seasonPoolAuthPda);
    const ixs = [];
    if (!(await exists(conn, seasonPool)))
        ixs.push((0, spl_token_1.createAssociatedTokenAccountIdempotentInstruction)(wallet.publicKey, seasonPool, seasonPoolAuthPda, cg));
    ixs.push(ix(ARENA, 'init_arena', [signer(wallet.publicKey), rw(arenaConfigPda), ro(web3_js_1.SystemProgram.programId)], new W().pubkey(oracle).pubkey(cg).pubkey(seasonPool).pubkey(ata(cg, treasury)).u64(cap).bytes()));
    await send(conn, wallet, ixs, `init_arena (oracle ${oracle.toBase58()}, cap ${cap / MICRO} $CG/day)`);
    if (oracle.equals(wallet.publicKey))
        console.warn('  !! battle oracle = deployer wallet — set BATTLE_ORACLE (backend resolver key) and call set_arena before G-1');
}
...
function anchorIds(section) {
    const toml = (0, node_fs_1.readFileSync)(new URL('../Anchor.toml', import.meta.url), 'utf8');
    const parts = toml.split(/^\[programs\.(\w+)\]\s*$/m);
    const out = {};
    for (let i = 1; i < parts.length; i += 2) {
        if (parts[i] !== section)
            continue;
        for (const m of parts[i + 1].matchAll(/^(\w+)\s*=\s*"([^"]+)"/gm))
            out[m[1]] = m[2];
    }
    return out;
}
async function mainnetPreflight(conn) {
    if (!MAINNET)
        return;
    const effective = { chip_core: CHIP_CORE, market: MARKET, staking: STAKING, arena: ARENA };
    const problems = [];
    for (const section of ['devnet', 'localnet']) {
        const ids = anchorIds(section);
        for (const [p, id] of Object.entries(effective)) {
            if (ids[p] === id.toBase58())
                problems.push(`${p} = ${id.toBase58()} is the [programs.${section}] placeholder — regenerate (npm run program-ids -- new / apply) and pass PROGRAM_${p.toUpperCase()}`);
        }
    }
    if (problems.length)
        throw new Error(`mainnet pre-flight (SEC-F05) FAILED:\n  - ${problems.join('\n  - ')}`);
    for (const p of ['chip_core', 'arena']) {
        const d = await (0, verify_deploy_ts_1.fetchDeployedProgram)(conn, effective[p]);
        console.log(`  ${p}: deployed at slot ${d.slot}, upgrade authority ${d.authority?.toBase58() ?? 'NONE (immutable)'}, sha256 ${(0, verify_deploy_ts_1.sha256hex)((0, verify_deploy_ts_1.trimPadding)(d.elf))}`);
        const v = (0, verify_deploy_ts_1.assessPins)(d.elf, 'mainnet', p);
        if (!v.ok)
            throw new Error(`mainnet pre-flight (SEC-F19) FAILED — ${p} on chain is not a mainnet build:\n  - ${v.problems.join('\n  - ')}\n  rebuild without the devnet/localnet feature, redeploy, then run setup again.`);
    }
...
async function main() {
    ...
    for (const [name, run] of steps) {
        if (only && name !== only && name !== 'mints')
            continue; // mints resolves addresses for every other step
        console.log(`\n▶ ${name}`);
        await run();
    }
