...
function parseSqlite(src: string, label: string): { tables: Tables; fts: string[]; sections: Map<string, string> } {
  ...
  const sections: Array<[number, string]> = [...src.matchAll(/^-- -{4,} ?(.+)$/gm)].map((m) => [m.index ?? 0, m[1]!.trim()]);
  const sectionAt = (pos: number) => [...sections].reverse().find(([p]) => p < pos)?.[1] ?? '(no section header)';
  const tables: Tables = new Map();
  const sect = new Map<string, string>();
  const fts: string[] = [];
  ...
  for (let m = re.exec(clean); m; m = re.exec(clean)) {
    ...
    if (/VIRTUAL/.test(m[0]!)) { fts.push(name); continue; }
    if (tables.has(name)) throw new Error(`duplicate CREATE TABLE ${name} (db.ts and a worker both declare it — pick one owner)`);
    ...
    for (const part of splitTopLevel(body)) {
      ...
      if (col?.[1]) cols.add(col[1].toLowerCase());
    }
...
function parsePrisma(src: string): Tables {
  ...
  const enums = new Set<string>([...src.matchAll(/^enum\s+([A-Za-z0-9_]+)/gm)].map((m) => m[1]!));
  const tables: Tables = new Map();
  for (const { table, body } of models.values()) {
    ...
    for (const rawLine of body.split('\n')) {
      ...
      if (rest.includes('@relation') || modelNames.has(base) || !(SCALARS.has(base) || enums.has(base))) continue;
...
type Drift = { key: string; kind: string; table: string; column?: string; note: string };

function diff(sqlite: Tables, prisma: Tables, fts: string[], sections: Map<string, string>): Drift[] {
  const out: Drift[] = [];
  for (const [t, cols] of sqlite) {
    ...
    for (const c of cols) {
      if (!prisma.get(t)!.has(c)) out.push({ key: `column:${t}.${c}:absent-in-prisma`, kind: 'column', table: t, column: c, note: `${sec(t)} writes ${t}.${c} and the target table has no such column` });
    }
  }
  for (const [t, cols] of prisma) {
    ...
    for (const c of cols) {
      if (!sqlite.get(t)!.has(c)) out.push({ key: `column:${t}.${c}:absent-in-sqlite`, kind: 'column', table: t, column: c, note: `${t}.${c} exists only in the target: SQLite derives it at read time, or the model is stale` });
    }
  }
  for (const t of fts) out.push({ key: `fts:${t}`, kind: 'fts', table: t, note: 'FTS5 is SQLite-only by definition; the Postgres equivalent is tsvector + GIN, so this is a porting task, not a schema to mirror' });
  return out.sort((a, b) => a.key.localeCompare(b.key));
}

const mode = process.argv.includes('--write') ? 'write' : 'check';
const ddlFiles = readdirSync(path.join(ROOT, 'backend/src'))
  .filter((f) => f.endsWith('.ts'))
  .map((f) => ({ label: f === 'db.ts' ? 'db.ts' : `src/${f}`, abs: path.join(ROOT, 'backend/src', f) }))
  .filter((f) => /CREATE TABLE/i.test(readFileSync(f.abs, 'utf8')))
  .sort((a, b) => (a.label === 'db.ts' ? -1 : b.label === 'db.ts' ? 1 : a.label.localeCompare(b.label)));
if (!ddlFiles.some((f) => f.label === 'db.ts')) { console.error(`::error::${DDL_FILE} declares no CREATE TABLE`); process.exit(2); }
const prismaSrc = readFileSync(path.join(ROOT, PRISMA_FILE), 'utf8');
const sqlite: Tables = new Map();
const fts: string[] = [];
const sections = new Map<string, string>();
for (const f of ddlFiles) {
  const parsed = parseSqlite(readFileSync(f.abs, 'utf8'), f.label);
  for (const [k, v] of parsed.tables) sqlite.set(k, v);
  for (const [k, v] of parsed.sections) sections.set(k, v);
  fts.push(...parsed.fts);
}
const sec = (t: string) => sections.get(t) ?? 'unknown';
...
if (mode === 'write') {
  ...
  for (const d of found) console.log(`  · ${d.key} — ${d.note}`);
  process.exit(0);
}

const baseline = existsSync(baselinePath)
  ? (JSON.parse(readFileSync(baselinePath, 'utf8')) as { allow: Array<{ key: string; note: string }> })
  : { allow: [] };
...
if (fresh.length || stale.length) {
  for (const d of fresh) console.log(`✗ new divergence: ${d.key} — ${d.note}`);
  for (const k of stale) console.log(`✗ stale baseline entry (the divergence is gone; delete it): ${k}`);
