...
const TREASURY_WALLET = new web3_js_1.PublicKey('HPMr5r9sS5ApWsPNJytZRLbm2jz1veFxTn1wepjAhtho');
/** Funding policy (bps of realised SKR revenue) — owner decision 15 / 10 / 5 %. Mirrors SKR_POOL_FUNDING. */
const POLICY = { packRevenue: 1500n, marketFeeTreasury: 1000n, servicesRevenue: 500n };
const SKR_MINT = new web3_js_1.PublicKey(process.env.SKR_MINT ?? REAL_SKR_MINT);
const STAKING_ID = new web3_js_1.PublicKey(process.env.STAKING_PROGRAM_ID ?? 'GCuGx7fnLcKnw1NWU4dLzQvnJWggMVniQ4u7EuMaQevA');
...
const skr = (micro) => `${(Number(micro) / 1e6).toLocaleString('en-US', { maximumFractionDigits: 6 })} SKR`;
const parseSkr = (s, fallback = 0n) => (s === undefined ? fallback : BigInt(Math.round(Number(s) * 1e6)));
const [emissionPda] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from('emission')], STAKING_ID);
const [poolPda, poolBump] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from('skr_pool')], STAKING_ID);
const vault = (0, spl_token_1.getAssociatedTokenAddressSync)(SKR_MINT, poolPda, true);
function loadWallet() {
...
const initSkrPoolIx = (admin, maxRootBudget) => new web3_js_1.TransactionInstruction({
    programId: STAKING_ID,
    keys: [signer(admin), ro(emissionPda), rw(poolPda), ro(SKR_MINT), ro(vault), ro(web3_js_1.SystemProgram.programId)],
    data: Buffer.concat([disc('init_skr_pool'), u64(maxRootBudget)]),
});
const fundSkrIx = (funder, amount) => new web3_js_1.TransactionInstruction({
    programId: STAKING_ID,
    keys: [signer(funder, false), rw(poolPda), rw((0, spl_token_1.getAssociatedTokenAddressSync)(SKR_MINT, funder)), rw(vault), ro(spl_token_1.TOKEN_PROGRAM_ID)],
    data: Buffer.concat([disc('fund_skr'), u64(amount)]),
});
const syncSkrPoolIx = () => new web3_js_1.TransactionInstruction({
    programId: STAKING_ID, keys: [rw(poolPda), ro(vault)], data: disc('sync_skr_pool'),
});
function decodePool(data) {
    const expected = Buffer.from((0, sha256_1.sha256)('account:SkrPool').subarray(0, 8));
    if (!data.subarray(0, 8).equals(expected))
        throw new Error('not a SkrPool account (discriminator mismatch)');
    let o = 8;
...
async function status(conn) {
    ...
    if (!info) {
        console.log('pool            : NOT INITIALISED — run `init`');
        return;
    }
    const p = decodePool(info.data);
    const bal = await (0, spl_token_1.getAccount)(conn, vault).then((a) => a.amount).catch(() => 0n);
    ...
    if (p.skrMint.toBase58() !== SKR_MINT.toBase58())
        console.warn(`!! pool mint ${p.skrMint.toBase58()} ≠ SKR_MINT env — check your environment`);
}
async function main() {
    ...
    switch (cmd) {
        case 'init': {
            ...
            if (!(await conn.getAccountInfo(emissionPda)))
                throw new Error('EmissionState not found — run init_emission first (pool admin = emission.admin)');
            if (await conn.getAccountInfo(poolPda)) {
                console.log('pool already initialised');
                return status(conn);
            }
            const ixs = [(0, spl_token_1.createAssociatedTokenAccountIdempotentInstruction)(wallet.publicKey, vault, poolPda, SKR_MINT), initSkrPoolIx(wallet.publicKey, maxRoot)];
        ...
        case 'fund': {
            ...
            if (amount <= 0n)
                throw new Error('usage: fund <amountSkr>');
        ...
        case 'plan': {
            ...
            if (!res.ok)
                throw new Error(`GET ${base}/v1/rewards/skr-pool → ${res.status}`);
            const pool = await res.json();
            const { funding: f } = pool;
            const due = {
                packs: (BigInt(f.revenue.packRevenueMicro) * POLICY.packRevenue) / 10000n,
                market: (BigInt(f.revenue.marketFeeTreasuryMicro) * POLICY.marketFeeTreasury) / 10000n,
                services: (BigInt(f.revenue.servicesRevenueMicro) * POLICY.servicesRevenue) / 10000n,
            };
            const dueTotal = due.packs + due.market + due.services;
            if (dueTotal.toString() !== f.dueMicro)
                console.warn(`!! backend policy (${JSON.stringify(f.policyBps)}) ≠ CLI policy ${JSON.stringify(POLICY, (_k, v) => typeof v === 'bigint' ? Number(v) : v)} — update one of them`);
            if (f.treasuryWallet !== TREASURY_WALLET.toBase58())
                console.warn(`!! backend treasury ${f.treasuryWallet} ≠ CLI ${TREASURY_WALLET.toBase58()}`);
        ...
        case 'test-mint': {
            ...
            const supply = parseSkr(arg, 1000000n * MICRO);
            const mint = await (0, spl_token_1.createMint)(conn, wallet, wallet.publicKey, null, 6);
            const ata = await (0, spl_token_1.getOrCreateAssociatedTokenAccount)(conn, wallet, mint, wallet.publicKey);
            await (0, spl_token_1.mintTo)(conn, wallet, mint, ata.address, wallet, supply);
