...
function steps(jobBody) {
    ...
    for (const l of body.slice(stepsAt + 1)) {
        ...
        if (indent(l) === mark && /^-\s/.test(l.trim())) {
            if (cur)
                out.push({ lines: cur });
            cur = [l];
            continue;
        ...
        if (indent(l) <= mark)
            break; // the job's next top-level key
        if (cur)
            cur.push(l);
    }
    if (cur)
...
function containerRanges(lines) {
    const cut = lines.findIndex((l) => /^jobs:\s*$/.test(l));
    if (cut < 0)
        return [];
    const starts = [];
...
function runBlocks(lines) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        for (let j = i + 1; j < lines.length; j++) {
            if (lines[j].trim() !== '' && indent(lines[j]) <= base)
                break;
            body.push(lines[j]);
        }
        ...
        const stepStart = (() => {
            for (let j = i - 1; j >= 0; j--)
                if (/^\s*-\s/.test(lines[j]) || /^\s{4}\S/.test(lines[j]))
                    return j;
            return i - 1;
        })();
        out.push({
...
const files = (0, node_fs_1.readdirSync)((0, node_path_1.join)(root, DIR)).filter((f) => /\.ya?ml$/.test(f)).sort();
const documented = new Set();
for (const line of (0, node_fs_1.readFileSync)((0, node_path_1.join)(root, EXAMPLE), 'utf8').split('\n')) {
 ...
 */
function annotationBackticks(text) {
    const out = [];
    for (const l of text.split('\n')) {
        if (!/::(error|warning|notice)/.test(l))
            continue;
        let quote = '';
        for (let k = 0; k < l.length; k++) {
            ...
            if (quote === '"') {
                ...
                if (ch === '`') {
                    out.push(`runs a command from inside annotation text — a backtick in a double-quoted workflow-command string (${l.trim().slice(0, 70)}…); drop the backticks or write the string in single quotes, where they are literal`);
                    break;
                }
                if (ch === '"')
                    quote = '';
...
const tmp = (0, node_fs_1.mkdtempSync)((0, node_path_1.join)((0, node_os_1.tmpdir)(), 'workflows-check-'));
for (const sh of (0, node_fs_1.readdirSync)((0, node_path_1.join)(root, 'scripts')).filter((f) => f.endsWith('.sh')).sort()) {
    for (const problem of annotationBackticks((0, node_fs_1.readFileSync)((0, node_path_1.join)(root, 'scripts', sh), 'utf8'))) {
        problems.push(`scripts/${sh}: ${problem}`);
    }
}
for (const file of files) {
    const src = (0, node_fs_1.readFileSync)((0, node_path_1.join)(root, DIR, file), 'utf8');
    ...
    for (const m of src.matchAll(VAR_REF)) {
        if (!documented.has(m[1]))
            problems.push(`${file}: reads vars.${m[1]}, which ${EXAMPLE} does not document`);
    }
    const inputs = dispatchInputs(lines);
    const usedInputs = new Set([...src.matchAll(INPUT_REF)].map((m) => m[1]));
    for (const name of usedInputs)
        if (inputs.size && !inputs.has(name))
            problems.push(`${file}: references inputs.${name}, which no workflow_dispatch block declares (an undeclared input is "", never an error)`);
    for (const name of inputs)
        if (!usedInputs.has(name))
            problems.push(`${file}: declares dispatch input "${name}" that no step reads`);
    if (file === 'images.yml') {
        ...
        for (const m of src.matchAll(SECRET_REF))
            problems.push(`${file}: reads secrets.${m[1]} — image publishing must not need one (a build-time secret ends up inside a public layer)`);
    }
    for (const job of jobs(lines)) {
        ...
        for (const block of blocks) {
            ...
            if (id && !emitted.has(id))
                emitted.set(id, new Set());
            if (block.lines.some((l) => /^\s*(?:-\s+)?uses:\s*/.test(l)) && id)
                external.add(id);
        ...
        for (const m of job.body.matchAll(OUT_REF)) {
            ...
            const [, sid, out] = m;
            if (!emitted.has(sid))
                problems.push(`${file} · job ${job.name}: reads steps.${sid}.outputs.${out} — no step in this job has id "${sid}"`);
            else if (!external.has(sid) && !emitted.get(sid).has(out))
                problems.push(`${file} · job ${job.name}: steps.${sid}.outputs.${out} is never written to $GITHUB_OUTPUT (that id emits: ${[...emitted.get(sid)].join(', ') || 'nothing'})`);
        }
    }
    runBlocks(lines).forEach((b, i) => {
        const script = b.body.replace(/\$\{\{[^}]*\}\}/g, 'DUMMY');
        const f = (0, node_path_1.join)(tmp, `${file}.${i}.sh`);
        (0, node_fs_1.writeFileSync)(f, script);
        const r = (0, node_child_process_1.spawnSync)('bash', ['-n', f], { encoding: 'utf8' });
        ...
        for (const problem of annotationBackticks(b.body))
            problems.push(`${file}: step "${b.name}" ${problem}`);
        if (r.status !== 0)
            problems.push(`${file}: bash -n rejects step "${b.name}" — ${(r.stderr || '').trim().split('\n')[0] || 'no message'}`);
        ...
        if (b.container && !b.shellBash) {
            dashChecked++;
            const rs = (0, node_child_process_1.spawnSync)('sh', ['-n', f], { encoding: 'utf8' });
            if (rs.status !== 0) {
                problems.push(`${file}: /bin/sh rejects step "${b.name}" — ${(rs.stderr || '').trim().split('\n')[0] || 'no message'}. ` +
                ...
                for (const [re, why] of BASHISMS) {
                    ...
                    if (m)
                        problems.push(`${file}: step "${b.name}" runs in a container, where /bin/sh is dash — ${m[0]} is ${why} (write POSIX, or declare shell: bash on the step)`);
                }
            }
        }
    });
...
if ((0, node_fs_1.existsSync)((0, node_path_1.join)(root, 'ops/deploy/.env.example')) && documented.size < 20) {
