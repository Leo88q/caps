...
function mdFiles(dir) {
    const abs = (0, node_path_1.resolve)(root, dir);
    if (!(0, node_fs_1.existsSync)(abs))
        return [];
    const out = [];
    for (const e of (0, node_fs_1.readdirSync)(abs)) {
        const rel = (0, node_path_1.join)(dir, e);
        if ((0, node_fs_1.statSync)((0, node_path_1.resolve)(root, rel)).isDirectory())
            out.push(...mdFiles(rel));
...
function resolveOwner(raw0) {
    ...
    if (m)
        return docFileByNumber(m[1]);
    if (raw.startsWith('docs/'))
        return null; // a docs/NN that does not exist is a miss, reported below
 ...
 */
function scan(f, text, lookForBare) {
    const owners = [];
    OWNER.lastIndex = 0;
    let o;
    while ((o = OWNER.exec(text))) {
        ...
        if (o[OWNER_SEC] && file) {
            ...
            for (const n of numsOf(o[OWNER_SEC]))
                if (!a.has(n))
                    misses.push({ file: f, line: lineOf(text, o.index), ref: `§${n}`, owner: file, detail: `анкора §${n} нет в ${file}` });
        }
    }
    if (!lookForBare)
        return;
    ...
    while ((m = BARE.exec(text))) {
        const at = m.index;
        if (owners.some((x) => at >= x.start && at < x.end))
            continue; // counted as part of an owner ref
...
selftest();
for (const f of files)
    scan(f, (0, node_fs_1.readFileSync)((0, node_path_1.resolve)(root, f), 'utf8'), true);
...
for (const dir of ['scripts', 'packages/economy/scripts', 'packages/client/scripts']) {
    ...
    if (!(0, node_fs_1.existsSync)(abs))
        continue;
    for (const e of (0, node_fs_1.readdirSync)(abs)) {
        if (!/\.(ts|mjs|js|sh)$/.test(e))
            continue;
        const rel = `${dir}/${e}`;
        if (rel === SELF)
            continue;
        scan(rel, (0, node_fs_1.readFileSync)((0, node_path_1.resolve)(root, rel), 'utf8'), false);
    }
}
if (asJson) {
...
if (misses.length) {
    for (const x of misses)
        console.error(`✗ ${x.file}:${x.line}: ${x.ref} — ${x.detail}`);
...
console.log(`doc refs ok: ${checked} ссылк(а/и) на разделы разрешены в ${files.length} md-файлах docs/ и ops/ ` +
