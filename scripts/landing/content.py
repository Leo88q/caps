...
T = {
  ...
  'eco.skr.p': ('Packs, listings, offers and extras accept SKR at the Pyth SKR/USD price; the program checks the feed id and a slippage cap in the same transaction. Only the genuine mint below is accepted — never a symbol lookup. Because the game cannot mint SKR, SKR prizes come from an on-chain prize pool funded by 25 % of SKR pack revenue: Seeker-week quests, season ladders and tournaments pay out in SKR through the same Merkle claims as $CG, capped per wallet. Wagers, fusion fees and staking stay in $CG.',
                'Паки, листинги, офферы и экстры принимают SKR по цене Pyth SKR/USD; программа проверяет id фида и лимит проскальзывания в той же транзакции. Принимается только подлинный минт ниже — никакого поиска по тикеру. Поскольку игра не может выпускать SKR, призы в SKR идут из on-chain призового пула, который пополняется 25 % выручки паков за SKR: квесты Seeker-недели, сезонные ладдеры и турниры выплачиваются в SKR через те же Merkle-клеймы, что и $CG, с капом на кошелёк. Ставки, fusion-fee и стейкинг остаются в $CG.'),
