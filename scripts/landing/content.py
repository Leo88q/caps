...
T = {
  ...
  'eco.p': ('Two currencies with two jobs. $CG is the game token: minted only by the program on a decaying schedule, burned by every sink. SKR — the Seeker token — is a payment rail: you pay with it, the treasury keeps it, the game never mints or burns it.',
            'Две валюты — две роли. $CG — игровой токен: его выпускает только программа по убывающему графику, а сжигает каждый sink. SKR — токен Seeker — это платёжный рельс: им платят, казна его хранит, игра его не выпускает и не сжигает.'),
  ...
  'eco.skr.h': ('SKR — the Seeker token as a payment rail', 'SKR — токен Seeker как платёжный рельс'),
  'eco.skr.p': ('Packs, listings, offers and extras accept SKR at the Pyth SKR/USD price; the program checks the feed id and a slippage cap in the same transaction. Only the genuine mint below is accepted — never a symbol lookup. Rewards, wagers and staking stay in $CG because the game cannot mint SKR.',
                'Паки, листинги, офферы и экстры принимают SKR по цене Pyth SKR/USD; программа проверяет id фида и лимит проскальзывания в той же транзакции. Принимается только подлинный минт ниже — никакого поиска по тикеру. Награды, ставки и стейкинг остаются в $CG, потому что игра не может выпускать SKR.'),
  ...
  'rules.5': ('If a reveal never arrives you cancel and get refunded after 300 slots. If a pack is bought and the app crashes, the pending pack is visible on your Home screen until it is opened or refunded.',
              'Если reveal так и не пришёл, вы отменяете покупку и получаете возврат через 300 слотов. Если пак куплен, а приложение упало, ожидающий пак виден на главном экране, пока не будет вскрыт или возвращён.'),
...
FAQ = [
  ...
  ('What if the reveal never comes?', 'Nothing is lost. A pending pack stays visible on your Home screen; after 300 slots you can cancel it and the payment is refunded by the program, not by support.',
   'Что, если reveal не придёт?', 'Ничего не теряется. Ожидающий пак остаётся на главном экране; через 300 слотов вы можете его отменить, и программа — не поддержка — вернёт оплату.'),
